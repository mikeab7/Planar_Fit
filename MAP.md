...
> **Generated by `scripts/build-map.mjs` — do not hand-edit the inventory.**
...
- **`src/workspaces/dashboard/components/CompsCard.jsx`** — TODO — describe
  - _exports_: `default (CompsCard)`
...
- **`src/workspaces/dashboard/components/DashboardCards.jsx`** — the six default Dashboard card renderers (pure presentational, token-only).
  - _exports_: `CardSkeleton`, `GoingQuietCard`, `JumpBackInCard`, `PipelineCard`, `ScheduleHealthCard`
...
- **`src/workspaces/dashboard/components/LocationsMapCard.jsx`** — TODO — describe
  - _exports_: `default (LocationsMapCard)`
...
- **`src/workspaces/dashboard/components/PursuitsCard.jsx`** — Pursuits dashboard card — table of open pursuits sorted by soonest contractual date.
  - _exports_: `PursuitsCard`
- **`src/workspaces/dashboard/components/RecentPlansCard.jsx`** — TODO — describe
  - _exports_: `RecentPlansCard`
- **`src/workspaces/dashboard/components/SinceLastHereCard.jsx`** — TODO — describe
  - _exports_: `SinceLastHereCard`
...
- **`src/workspaces/dashboard/lib/buildingYield.js`** — Dependency-free gross building SF sum from raw element rows (the Pursuits card's Yield column).
  - _exports_: `buildingCountBySite`, `grossBuildingSqft`, `yieldBySite`
- **`src/workspaces/dashboard/lib/compsCardModel.js`** — TODO — describe
  - _exports_: `bandSentenceLabel`, `buildCompsCardData`, `buildPeerSet`, `compHeadlineRate`, `compScaleKey`, `compScaleLayout`, `compSizeSf`, `countyEntry`, `countyLabel`, `countyNameWords`, `DEFAULT_LEASE_PERIOD`, `formatRateValue`, `leaseRateUnit`, `MIN_PEERS_FOR_SCALE`, `mostRecentlyAddedComp`, `peerComparisonSentence`, `periodWords`, `relativeTimeLabel`, `scaleLabel`, `SIZE_BANDS`, `sizeBandFor`, `TYPE_LABEL`
- **`src/workspaces/dashboard/lib/dashboardCompsFetch.js`** — the one minimal `comps` read (type counts) the Comps summary card needs.
  - _exports_: `fetchAllCompsForCard`, `fetchCompsForMap`
- **`src/workspaces/dashboard/lib/dashboardCompsRecentFetch.js`** — TODO — describe
  - _exports_: `fetchRecentComps`
- **`src/workspaces/dashboard/lib/dashboardDates.js`** — Shared date-formatting helpers (short date label, days-until) for the dashboard cards.
  - _exports_: `dayDividerLabel`, `dayKey`, `daysUntil`, `formatShortDate`, `shortAge`, `spanWords`
...
- **`src/workspaces/dashboard/lib/dashboardLayout.js`** — pure model for the arrangeable card grid: catalog, default layout, add/remove/reorder/resize.
  - _exports_: `addCard`, `appendNewCatalogCards`, `applyGridChange`, `availableToAdd`, `CARD_DEFS`, `CARD_KEYS`, `DEFAULT_LAYOUT`, `DEFAULT_ORDER`, `dismissCard`, `GRID_COLS`, `narrowOrder`, `newCatalogCards`, `normalizeDismissed`, `normalizeLayout`, `removeCard`, `resetLayout`, `toRglItem`, `undismissCard`
- **`src/workspaces/dashboard/lib/dashboardMapMarkers.js`** — TODO — describe
  - _exports_: `mapMarkers`, `missingLocationCount`, `openPipelineProjects`
- **`src/workspaces/dashboard/lib/dashboardNotesRecentFetch.js`** — TODO — describe
  - _exports_: `fetchRecentNotePages`, `openingWords`
...
- **`src/workspaces/dashboard/lib/dashboardScheduleFetch.js`** — the one `planar_data` read (the embedded Scheduler's own document) the Schedule health card needs.
  - _exports_: `fetchScheduleLastWriteAt`, `fetchScheduleProjects`
- **`src/workspaces/dashboard/lib/dashboardSinceLastHerePrefs.js`** — TODO — describe
  - _exports_: `loadSinceLastHere`, `saveSinceLastHere`
...
- **`src/workspaces/dashboard/lib/dashboardYieldFetch.js`** — Paged site_elements read scoped to specific plan ids, for the Pursuits card's Yield column.
  - _exports_: `fetchElementsForSites`
- **`src/workspaces/dashboard/lib/labelCollide.js`** — TODO — describe
  - _exports_: `boxesOverlap`, `resolveLabelVisibility`
- **`src/workspaces/dashboard/lib/needsAttentionList.js`** — Pure flat needs-attention task list + per-project totals, across every schedule project.
  - _exports_: `attentionBarFraction`, `needsAttentionList`, `needsAttentionTotals`
- **`src/workspaces/dashboard/lib/pursuitsList.js`** — Pure Pursuits table model: soonest-contractual-date sort, quiet-days fold, tone helpers.
  - _exports_: `isQuietEmphasized`, `pursuitsTable`, `QUIET_EMPHASIS_DAYS`, `quietDaysByGroupFromRecency`, `quietDaysByGroupFromRows`
- **`src/workspaces/dashboard/lib/recentPlans.js`** — TODO — describe
  - _exports_: `pickRecentPlans`
- **`src/workspaces/dashboard/lib/recentPlansLayout.js`** — TODO — describe
  - _exports_: `countForMode`, `MIN_THUMB_PX`, `recentPlansLayoutMode`
- **`src/workspaces/dashboard/lib/recentPlansThumbnailFallback.js`** — TODO — describe
  - _exports_: `lazyGenerateThumbnail`
- **`src/workspaces/dashboard/lib/relativeTime.js`** — TODO — describe
  - _exports_: `relativeTimeShort`
...
- **`src/workspaces/dashboard/lib/sinceLastHereFeed.js`** — TODO — describe
  - _exports_: `buildSinceLastHereFeed`, `capRowsFairlyByKind`, `compAddedSubline`, `KIND_META`
...
- **`src/workspaces/model/lib/rowLayout.js`** — Pure variable-row-height offset/search math behind SheetView's virtualized window (cumulative offsets + binary search), DOM-free for unit testing.
  - _exports_: `buildRowOffsets`, `rowAtOffset`, `visibleRowRange`
- **`src/workspaces/model/lib/sheetColorMode.js`** — The input/formula/cross-sheet-link auto-colour toggle's per-project view preference (localStorage, like sheetZoom.js).
  - _exports_: `DEFAULT_AUTO_COLOR`, `readAutoColor`, `writeAutoColor`
...
- **`src/workspaces/model/lib/sheetModel.js`** — The pure spreadsheet data model: columns/cells/formulas, cell addressing, and every mutator (all pure, undo-stack-ready).
  - _exports_: `activeSheetEntry`, `activeSheetIndex`, `addColumn`, `addSheet`, `addSheetWithContent`, `applyBorder`, `applyPaintedStyle`, `applyToActiveSheet`, `blankRange`, `cellKey`, `clearFormatting`, `colAt`, `colLetterName`, `columnIndexByName`, `commitCellText`, `createSheet`, `createWorkbook`, `DEFAULT_ROW_H`, `deleteColumn`, `deleteRowAt`, `deleteSheet`, `duplicateSheet`, `ensureColumnCount`, `formatAt`, `insertColumnAt`, `insertRowAt`, `isFormulaText`, `isInconsistencyDismissed`, `mergeAt`, `mergeRange`, `migrateSheet`, `migrateWorkbook`, `padRowCount`, `paintedStyleAt`, `rawAt`, `renameColumn`, `renameSheet`, `reorderSheet`, `rowHeightAt`, `setActiveSheet`, `setCellStyle`, `setColumnWidth`, `setFreeze`, `setInconsistencyDismissed`, `setNumberFormat`, `setRaw`, `setRowHeight`, `SHEET_VERSION`, `sheetsDiverge`, `sortRange`, `styleAt`, `unmergeAt`, `unsupportedFormulaAt`, `usedRangeEnd`, `WORKBOOK_VERSION`, `workbookDeleteColumn`, `workbookDeleteRowAt`, `workbookHasContent`, `workbookInsertColumnAt`, `workbookInsertRowAt`
...
- **`src/workspaces/notes/components/NoteEditor.jsx`** — One note page (title · toolbar · document) and the module's ONLY editor-engine import — the lazy boundary. Snapshots the document as plain JSON at edit time so the flush never queries a torn-down instance.
  - _exports_: `default (NoteEditor)`, `noteTitleFontPx`, `PASTE_MODE_META`, `placeMenu`
...
- **`src/workspaces/notes/lib/notesAnchorNode.js`** — the `noteAnchor` schema node: a block that stays where it was double-clicked. Position is two numbers ON the node (unscaled document pixels), so it cannot crawl, cannot leak alignment, leaves no padding paragraphs, and rides the document into storage, sync and the PDF. Plus the pure `clampAnchor`.
  - _exports_: `ANCHOR_EDGE_PAD`, `ANCHOR_MIN_HEIGHT`, `ANCHOR_MIN_WIDTH`, `ANCHOR_WIDTH`, `anchorExtent`, `anchorExtentLeft`, `anchorExtentTop`, `anchorExtentX`, `anchorPosAtSelection`, `default`, `fitAnchorBox`, `fitAnchorWidth`, `NoteAnchor`, `placeAnchor`
...
- **`src/workspaces/notes/lib/notesBoxResize.js`** — The placed box's geometry: the floors, the edge pad, and the pure rule for resizing one from any of its eight handles (corners hold the ratio, edges stretch, Shift inverts, the edges you are not holding never move). Also decides which handles a box may offer, read off its content.
  - _exports_: `ANCHOR_EDGE_PAD`, `ANCHOR_MIN_HEIGHT`, `ANCHOR_MIN_WIDTH`, `ANCHOR_WIDTH`, `anchorExtent`, `anchorExtentLeft`, `anchorExtentTop`, `anchorExtentX`, `default`, `HANDLE_CURSOR`, `HANDLES`, `handlesFor`, `hasFixedHeight`, `isCorner`, `locksAspect`, `moveAnchorPoint`, `MOVES_ORIGIN`, `resizeBox`
...
- **`src/workspaces/notes/lib/notesConflictDiff.js`** — Pure word-level diff behind ConflictCompare, with a line-level then a linear prefix/suffix fallback so it never builds an unbounded table on an outsized document.
  - _exports_: `diffHasChanges`, `diffNoteText`, `lcsAlign`, `sideOps`
...
- **`src/workspaces/notes/lib/notesDuplicates.js`** — pure detector for THE SAME NOTE LIVING IN TWO PROJECTS: word-pair Dice similarity over normalised text, same-project and empty pages deliberately excluded, groups (not pairs) out.
  - _exports_: `duplicateNotice`, `findCrossProjectDuplicates`, `joinMentions`, `keepCopyLabel`, `MIN_TEXT_CHARS`, `NEAR_DUPLICATE_SIMILARITY`, `normalizeText`, `numberWord`, `pageMention`, `sentenceCase`, `shingles`, `similarity`, `truncateMention`
...
- **`src/workspaces/notes/lib/notesFontFamily.js`** — TODO — describe
  - _exports_: `familyKey`, `firstFamily`, `fontDisplayLabel`, `matchFontOption`
...
- **`src/workspaces/notes/lib/notesMixedSelection.js`** — PURE: does a text selection agree on one font size/block style, or is it MIXED (`getAttributes`/`isActive` only ever answer for a caret's single position, not a range that can honestly disagree with itself) — and what a toolbar control should display for either case.
  - _exports_: `formatDisplayValue`, `MIXED`, `selectionAlignments`, `selectionBlockShapes`, `selectionFontFamilies`, `selectionFontSizes`, `selectionLineHeights`, `selectionListKinds`, `selectionMarkAttrs`, `selectionMarkPresence`, `togglePressed`, `uniformValue`
...
- **`src/workspaces/notes/lib/notesPasteInherit.js`** — TODO — describe
  - _exports_: `inheritedStyle`, `pushDownInherited`
...
- **`src/workspaces/notes/lib/notesPrint.js`** — The Notes print / Save-as-PDF sheet — a pure HTML document builder whose paper CSS mirrors the screen's editor CSS, plus the hidden-iframe driver that opens the print dialogue.
  - _exports_: `buildPrintDocument`, `pageAnchorExtentLeftPx`, `pageAnchorExtentPx`, `pageAnchorExtentTopPx`, `printHtmlDocument`
...
- **`src/workspaces/notes/lib/notesResolvedValue.js`** — TODO — describe
  - _exports_: `defaultFontLabel`, `isNonValue`, `resolvedColor`, `resolvedColorsAgree`
...
- **`src/workspaces/notes/lib/notesSpacing.js`** — how far apart the lines are: a BLOCK property (paragraph/heading), written into the markup so it saves, syncs and prints with no second stylesheet
  - _exports_: `BLOCK_SPACES`, `blockFontSize`, `COMFORTABLE_LINE`, `DEFAULT_DENSITY`, `DENSITIES`, `densityFor`, `densityStyle`, `fontSizePx`, `LINE_SPACINGS`, `SINGLE`, `spacingFromElement`, `spacingLabel`, `spacingStyle`
...
- **`src/shared/cloud/writeFailureLog.js`** — Durable (localStorage) record of a cloud write that failed, surviving a same-event auto-reload that can outrun an in-memory banner before it paints
  - _exports_: `clearAllCloudWriteFailures`, `clearCloudWriteFailure`, `inferEntryKind`, `PRE_FIX_RETRY`, `readCloudWriteFailures`, `recordCloudWriteFailure`, `replayCloudWriteFailures`, `retryCloudWriteFailures`, `WHAT_DATES`, `WHAT_RENAME`, `WHAT_STATUS`
...
- **`src/shared/comps/components/CompsPanel.jsx`** — Leasing Comps right-side panel (B711328): list/detail/create-edit for land, building-sale and lease comps; owner-only Edit/Delete, empty fields never render
  - _exports_: `_resetPinAddrCache`, `CompDetail`, `CompForm`, `CompRow`, `default (CompsPanel)`, `persistPinAddr`, `PIN_ADDR_STORAGE_KEY`, `PIN_ADDR_STORAGE_MAX`, `pinCacheKey`, `readPinAddrStorage`
...
- **`src/shared/comps/lib/compSheetColumns.js`** — pure column model for the comp-entry spreadsheet: per-cell get/set, polymorphic fields, display formatting, fill-down/paste-spill
  - _exports_: `absorbPasteIntoSheet`, `applyCellEdit`, `BASIS_OPTIONS`, `cellPlaceholder`, `cellState`, `columnIndex`, `computeFlexWidths`, `fillDownColumn`, `formatNumberDisplay`, `frozenLeftOffsets`, `groupLabelIsRedundant`, `GROUPS`, `isUnfilledRow`, `matchOption`, `optionsForColumn`, `PERIOD_OPTIONS`, `sanitizeNumericInput`, `saveButtonLabel`, `SHEET_COLUMNS`, `spillPaste`, `TYPE_OPTIONS`, `UNIT_OPTIONS`, `visibleColumnIndices`, `widthFor`
...
- **`src/shared/comps/lib/compsRatePeriodPrefs.js`** — TODO — describe
  - _exports_: `_normalizePeriod`, `DEFAULT_COMPS_RATE_PERIOD`, `loadCompsRatePeriod`, `saveCompsRatePeriod`
...
- **`src/shared/comps/lib/pinAddrCache.js`** — TODO — describe
  - _exports_: `persistPinAddr`, `PIN_ADDR_STORAGE_KEY`, `PIN_ADDR_STORAGE_MAX`, `pinCacheKey`, `readPinAddrStorage`
...
- **`src/shared/formula/formula.js`** — Excel-style formula engine (tokenize/parse/evaluate) powering scheduler user-defined columns; no eval, structured [Column] refs, byte-synced into the Sequence iframe
  - _exports_: `BLANK`, `colLettersToNum`, `colNumToLetters`, `compareValues`, `DEFAULT_CALENDAR`, `dropFormulaSheetRefs`, `errVal`, `evaluateFormula`, `extractRefs`, `formatValue`, `formatValueColor`, `FORMULA_ERRORS`, `FormulaError`, `FUNCTION_HELP`, `FUNCTION_NAMES`, `FUNCTIONS`, `isBlank`, `isDate`, `isErrVal`, `isFormulaError`, `isoToSerial`, `makeDate`, `MAX_COL`, `MAX_ROW`, `numToGeneralStr`, `parse`, `parseFormula`, `parseLooseDate`, `parseRefText`, `planFormulaColumns`, `rewriteFormulaForCopy`, `rewriteFormulaForSheetRename`, `rewriteFormulaForStructuralShift`, `serialToISO`, `serialToYMD`, `toBool`, `toDateSerial`, `tokenize`, `toNumber`, `toStr`, `weekdayOf`, `ymdToSerial`
...
- **`src/shared/mapNotes/components/MapNoteEditor.jsx`** — TODO — describe
  - _exports_: `default (MapNoteEditor)`
- **`src/shared/mapNotes/lib/mapNoteMarkerIcon.js`** — TODO — describe
  - _exports_: `mapNoteMarkerSize`, `mapNoteMarkerSvg`, `NOTE_MARKER_COLOR`, `NOTE_MARKER_INK`
- **`src/shared/mapNotes/lib/mapNotes.js`** — TODO — describe
  - _exports_: `emptyMapNote`, `mapNoteHasText`, `mapNoteHeadline`, `mapNoteToRow`, `NOTE_BODY_MAX`, `NOTE_TITLE_MAX`, `rowToMapNote`, `sortMapNotesByRecency`, `validateMapNote`
- **`src/shared/mapNotes/lib/mapNotesStore.js`** — TODO — describe
  - _exports_: `deleteMapNote`, `fetchAllMapNotes`, `fetchDeletedMapNotes`, `insertMapNote`, `permanentlyDeleteMapNote`, `restoreMapNote`, `supabase`, `updateMapNote`
...
- **`src/shared/profile/profileRowCache.js`** — TODO — describe
  - _exports_: `getProfileRow`, `invalidateProfileRow`
...
- **`src/shared/projects/docProjectLiveness.js`** — TODO — describe
  - _exports_: `docProjectIsDead`, `liveProjectIds`, `openableProjectId`
- **`src/shared/projects/projectModel.js`** — Pure project-model helpers: collapse site records into one project per site-group, name-match suggest, dropdown filter, and relative-time formatting for the breadcrumb switcher
  - _exports_: `applyFrozenOrder`, `DELETED_RETENTION_DAYS`, `distanceFeetBetween`, `filterProjects`, `findProjectAtOrigin`, `groupProjects`, `hasSavedProjectRecord`, `markProjectFreshlyMinted`, `normalizeProjectName`, `projectGateStatus`, `relTime`, `resolveControlledId`, `resolveCurrentName`, `SAME_GROUND_FT`, `shortenDisplayName`, `suggestNameMatch`, `unionProjectLists`, `wasProjectFreshlyMinted`, `withCurrentProject`
...
- **`src/shared/schedule/scheduleOwnership.js`** — TODO — describe
  - _exports_: `describeScheduleDelete`, `isOrgOwned`, `isSiteOwned`, `migrateScheduleOwnership`, `nameCollision`, `normalizeName`, `normalizeScheduleOwnership`, `ORG_OWNER_KEY`, `ORG_OWNER_LABEL`, `OWNER_KIND_ORG`, `OWNER_KIND_SITE`, `ownerKeyOf`, `ownerOf`, `partitionSchedules`, `pruneOrphanScheduleRefs`, `pruneScheduleRefs`, `scheduleList`, `schedulesForOwner`, `suggestScheduleName`, `validateNewSchedule`
...
- **`src/shared/sitePlans/lib/overlaySiteMatch.js`** — matches a placed site plan to an existing site by footprint containment (not the comp radius), falling back to an exact title match
  - _exports_: `findMatchingSiteForOverlay`, `SITE_MATCH_BUFFER_FT`
...
- **`src/shared/ui/Chip.jsx`** — TODO — describe
  - _exports_: `default (Chip)`
- **`src/shared/ui/chromeDock.js`** — TODO — describe
...
- **`src/shared/ui/FloatingNotice.jsx`** — The one shared primitive owning position for every floating, app-level notification: bottom-centered, stacked via a shared portal host, max-width clamped, clears an open mobile bottom sheet
  - _exports_: `default (FloatingNotice)`, `NOTICE_BOTTOM`, `NOTICE_GAP`, `NOTICE_MAX_WIDTH`, `NOTICE_Z`, `noticeBottomOffset`
...
- **`src/shared/ui/menuLayers.js`** — TODO — describe
  - _exports_: `layersOf`, `MENU_LAYER_ATTR`, `pressBelongsToHigherMenu`, `pressIsAboveLayer`
...
- **`src/shared/ui/RowInfo.jsx`** — TODO — describe
...
- **`src/shared/ui/Toast.jsx`** — reusable non-blocking toast stack (B673): auto-dismiss with hover-hold, action slot, cap + "+n more", theme tokens; pure pushToastPure/visibleToasts for tests
  - _exports_: `countByDedupeKey`, `pushToastPure`, `TOAST_CAP`, `TOAST_TTL_MS`, `ToastHost`, `useToasts`, `visibleToasts`
...
- **`src/workspaces/site-planner/components/DealDatesForm.jsx`** — Inline popover form for a pursuit's feasibility/LOI/closing dates.
  - _exports_: `default (DealDatesForm)`
...
- **`src/workspaces/site-planner/lib/appraisal.js`** — Pure CAD-attribute curation: regex-maps raw county/TxGIO parcel columns to labelled owner/value/acreage/use rows for both panels
  - _exports_: `ADDR_LINE_RE`, `APPR_FIELDS`, `apprAll`, `apprRows`, `apprVal`, `findAttr`, `isPlaceholderValue`, `MAILING_KEY_RE`, `mailingAddressValues`, `ownerName`, `PARCEL_CARD_PRIMARY_LABELS`, `PARCEL_PANEL_PRIMARY_LABELS`, `parcelCardRows`, `parcelPanelRows`, `prettyKey`, `siteNameFromParcel`, `SITUS_FIELD`, `SITUS_LADDER`, `situsAddress`, `situsKey`, `splitCuratedRows`, `tidyAddressLabel`
...
- **`src/workspaces/site-planner/lib/cloudSync.js`** — RLS-scoped Supabase site read/write: per-tab version CAS + thin-clobber guard, keepalive push, delete-tombstone reconcile
  - _exports_: `_lastHeaderSig`, `_siteVersions`, `clearSiteVersions`, `cloudCheckDeleted`, `cloudDelete`, `cloudDeletedRows`, `cloudDeleteGroup`, `cloudElementRecency`, `cloudHardDelete`, `cloudList`, `cloudParcelRows`, `cloudRestore`, `cloudUpsert`, `fetchSiteForReconcile`, `hasAnyLiveSites`, `headerSig`, `interpretDelete`, `keepaliveCloudPush`, `siteRowFor`, `slimForCloud`
...
- **`src/workspaces/site-planner/lib/compParcelAnchor.js`** — B941152: the pure comp-anchor derivation for a real-parcel map selection of any size — one Polygon for a single parcel, a MultiPolygon of every selected parcel's rings for several (never just the last), every account id joined, county fallback, and the toolbar's own already-computed acreage carried through
  - _exports_: `compAnchorFromSelection`, `parcelAnchorFromSelection`, `parcelApnFromSelection`, `parcelCountyFromSelection`, `parcelGeomFromSelection`
...
- **`src/workspaces/site-planner/lib/decideBar.js`** — TODO — describe
  - _exports_: `decideTargetOf`, `orderVerbs`, `verbLabel`
...
- **`src/workspaces/site-planner/lib/deviceShape.js`** — TODO — describe
  - _exports_: `isPhoneShape`
...
- **`src/workspaces/site-planner/lib/elementSync.js`** — the per-element write engine (B671): diffs live collections vs a shadow map, batches rev-guarded commits, LWW conflict events, dirty-queue backoff
  - _exports_: `createElementSync`, `isOwnWrite`, `nextOpForStatus`, `RETRYABLE_UNCHANGED`, `semanticallyEqual`, `stableStringify`
...
- **`src/workspaces/site-planner/lib/featureTarget.js`** — which feature a canvas double-click is about, resolved at the SVG root off the live hit stack (plus the dimension-number-over-body test)
  - _exports_: `ACTION_ATTR`, `CHROME_ATTR`, `EL_DIM_ATTR`, `FEATURE_ATTR`, `FEATURE_KINDS`, `featuresBeneath`, `featureStack`, `gestureAnchorTarget`, `HANDLE_ATTR`, `nextPickIndex`, `parseFeatureKey`, `PICK_SAME_POINT_PX`, `pressIsOverElementBody`, `resolveDoubleClickTarget`, `stackAtPoint`, `stackEntries`, `stackHoldsFeature`
...
- **`src/workspaces/site-planner/lib/mapChromeStack.js`** — the ONE map-overlay stacking model (an open panel outranks map chrome — Leaflet controls, scale bar) plus the available-room panel height
  - _exports_: `COMPS_TOGGLE_CLEARANCE_PX`, `LEAFLET_CONTROL_Z`, `MAP_CHROME_Z`, `MAP_OVERLAY_BAR_H_PX`, `MAP_OVERLAY_CHIP_H_PX`, `MAP_OVERLAY_TOP_PX`, `panelMaxHeight`, `SCALE_BAR_CLEARANCE_PX`, `TOP_RIGHT_ROW_RESERVE_PX`, `ZOOM_CONTROL_CLEARANCE_PX`, `zoomStackBottomPx`
...
- **`src/workspaces/site-planner/lib/mapLock.js`** — THE projection welding the planner's feet frame to the Web-Mercator basemap — scaled-Mercator feet↔lat/lng plus the matching ppf↔zoom, both anchored at the site origin
  - _exports_: `basemapWrapPoint`, `basemapWrapPointTransformed`, `exactContainerPoint`, `feetToLatLngPair`, `FT_PER_DEG`, `ftPerDeg`, `hasRegisterableContainer`, `invMercDeg`, `lngLatToFeet`, `lockOffsetPx`, `mercDeg`, `ppfToZoom`, `REGISTRATION_SANITY_PX`, `registrationLayoutMayHaveChanged`, `registrationShift`, `sanitizeShift`, `tileNwFeet`, `viewValuesEqual`, `zoomToPpf`
...
- **`src/workspaces/site-planner/lib/overlayPdf.js`** — Site-plan overlay rasterizer: lazily reuses Doc Review PDF.js to render a dropped PDF/image page to a white-knockout PNG data URL, reads its scale note, classifies sheet size, rebuilds from stored bytes
  - _exports_: `baseRasterScale`, `chooseOverlayRasterScale`, `HIRES_CACHE_PER_OVERLAY`, `isDxfFile`, `isPdfFile`, `knockoutCanvas`, `knockoutNearWhite`, `MAX_RERASTER_DIM`, `openOverlayFile`, `overlayRasterKey`, `rasterizePage`, `rasterizePageHiRes`, `rasterizeStoredDxf`, `rasterizeStoredPdf`, `RERASTER_LADDER`
...
- **`src/workspaces/site-planner/lib/parcelOpacityGuard.js`** — TODO — describe
  - _exports_: `guardRasterOpacity`
...
- **`src/workspaces/site-planner/lib/planThumbnail.js`** — TODO — describe
  - _exports_: `planThumbnailSvg`, `simplifyRing`
...
- **`src/workspaces/site-planner/lib/projectName.js`** — THE authority for what a project (site group) is called: resolves the one authoritative name across a group's plans (rename-stamp first, legacy majority second, ambiguous never guessed), reconciles a split group, and supplies the write-path correction that stops a stale plan re-publishing an old name.
  - _exports_: `byGroup`, `carriesEmptyRenameStamp`, `groupKeyOf`, `maxStampOf`, `nameAuthority`, `normalizeRenameStampForWrite`, `reconcileGroupNames`, `renameStamp`, `resolveNameFor`
...
- **`src/workspaces/site-planner/lib/roadCrossSection.js`** — A road's cross-section as an ordered list of typed bands (travel lane, median, turn lane, shoulder, parking, bike, sidewalk, etc.), curb-to-curb/paved/ROW width derivations, lane-striping seams, and built-in presets
  - _exports_: `BAND_FILL_OPACITY`, `BAND_FILL_TOKEN`, `BAND_TYPE_BY_KEY`, `BAND_TYPES`, `bandExtents`, `bandLayout`, `bandStripeMarks`, `bandStripeMarksWithWidth`, `bandTypeOf`, `BUILT_IN_XSECTION_PRESETS`, `curbToCurbWidth`, `DEFAULT_BAND_TYPE`, `designatedRowFt`, `hasXSection`, `imperviousCorrectionWidths`, `makeXSection`, `MIN_BAND_WIDTH_FT`, `normalizeBands`, `parseWidthDraft`, `pavedWidth`, `pavementArea`, `rowMarginFt`, `rowMarginsBySide`, `rowWidth`, `XSEC_BAND_FILL_MIN_PX`, `XSEC_SF_PER_SY`, `XSEC_STRIPE_MIN_PX`, `xsectionFromRoad`
...
- **`src/workspaces/site-planner/lib/siteGeometry.js`** — Shared pure geometry primitives behind a site's drawn area math: element footprints, curb bands, road strip/pavement area, junction detection and the per-plan roundabout resolution
  - _exports_: `carStalls`, `CURB`, `CURB_6`, `CURB_TYPES`, `curbAreaOf`, `curbEdgesOf`, `curbHost`, `curbWidthOf`, `elCorners`, `estStalls`, `estTrailers`, `isCenterlineRoad`, `outwardCurbEdge`, `polyArea`, `ringOf`, `roadCurbLines`, `roadCurbWidth`, `roadDefaultRadius`, `roadDenseCenterline`, `roadImperviousArea`, `roadJunctionVerticesOf`, `roadStripArea`, `roadStripRing`, `rot2`, `roundaboutsForSite`, `sidewalkBetween`, `SQFT_PER_ACRE`, `TEE_COINCIDE_FT`, `TEE_COINCIDE_MAX_FT`, `teeCoincideFt`, `teeTargetOf`, `trailerStalls`
...
- **`src/workspaces/site-planner/lib/siteModel.js`** — Canonical per-plan Site Model schema v10: createSiteModel/migrate, semantic selectors, cross-copy union merge with delete-tombstones, and bonded-child/dog-ear/road-centerline load-time repairs
  - _exports_: `activeParcelsOf`, `ANNOTATION_KINDS`, `annotationsOf`, `bondedChildRot`, `buildingNumbers`, `constraintsOf`, `contentCount`, `countJunkEntries`, `createSiteModel`, `crossSectionsOf`, `DEFAULT_ROLE`, `developableArea`, `EASEMENT_KINDS`, `easementsOf`, `elementsOf`, `exclusionZonesOf`, `ID_BOND_KEYS`, `impossibleStacks`, `isBuilding`, `lineageConflicts`, `mergeSiteContent`, `migrate`, `migrateRoads`, `missingBondSiblings`, `normalizeBondedChildren`, `normalizeCrossHostBonds`, `normalizeHostRuns`, `normalizeOrphanWallPads`, `normalizeZoneAlongLen`, `normRole`, `offAnchor`, `orphanWallPads`, `parcelAncestors`, `parcelChildrenMap`, `parcelDescendants`, `parcelDisplayInfo`, `parcelDrawingsOf`, `parcelOutline`, `parcelsOf`, `parcelSplitNames`, `quarterOffset`, `rectRoadEndpoints`, `RESTORED_STRIP_W_FT`, `roadStripBBox`, `roadTravelWidth`, `ROLE_META`, `roleOf`, `ROLES`, `setbacksOf`, `SHARE_MIRROR_FIELDS`, `shareMirrorOf`, `sheetOverlaysOf`, `SITE_MODEL_VERSION`, `STATUS_META`, `STATUSES`, `statusOf`, `strandedFromHost`, `teamShareOf`, `toMs`, `utilitiesOf`, `UTILITY_KINDS`, `withShareMirror`
...
- **`src/workspaces/site-planner/lib/siteStatus.js`** — B927105: the Site Model's schema-version + deal-status constants, split out of siteModel.js (dependency-free) so a caller that only needs a status label doesn't import the geometry engine
  - _exports_: `DEFAULT_ROLE`, `DEFAULT_STATUS`, `isLegacyRecord`, `LEGACY_STATUS`, `normRole`, `normStatus`, `ROLE_META`, `roleOf`, `ROLES`, `SITE_MODEL_VERSION`, `STATUS_META`, `STATUSES`, `statusOf`
- **`src/workspaces/site-planner/lib/siteThumbnail.js`** — TODO — describe
  - _exports_: `generateAndStoreThumbnail`, `refreshSiteThumbnailFromModel`
...
- **`src/workspaces/site-planner/lib/sourceHealth.js`** — Per-source circuit breaker for county parcel servers: track consecutive failures, open/cooldown/half-open, filter healthy candidates, and decide the honest statewide-backup badge
  - _exports_: `filterHealthyCandidates`, `isSourceOpen`, `isStatewideBackup`, `recordSourceResult`, `resetSourceHealth`, `SOURCE_COOLDOWN_MS`, `SOURCE_FAIL_THRESHOLD`, `SOURCE_SLOW_MS`, `sourceCooldownMs`
...
- **`src/workspaces/site-planner/lib/storage.js`** — Multi-site persistence layer: localStorage primary with per-user cloud mirror, content-union pull merge, per-tab resurrection guards, and an IndexedDB-backed version-history ring
  - _exports_: `_readSiteTombs`, `_recentlyDeleted`, `_resetHistoryForTest`, `activeUid`, `AUTOSAVE_KEY`, `backupNow`, `binOrphanedTrackedSite`, `checkProjectDeletionStatus`, `clearCloudCache`, `clearHistory`, `clearRecentlyDeleted`, `clearSiteTombstone`, `DELETED_RETENTION_DAYS`, `deleteSite`, `deleteSiteGroup`, `discardLegacySite`, `ensureProjectRow`, `getCurrentSiteId`, `getVersion`, `groupCountDivergence`, `groupOf`, `hasAnyLocalSites`, `importLegacyIntoCloud`, `importOneSiteToCloud`, `initHistoryStore`, `isCloudActive`, `isEmptySite`, `keepaliveFlushSite`, `legacySitesList`, `listDeletedPlansInGroup`, `listDeletedProjects`, `listVersions`, `loadAutosave`, `loadPlansOfGroup`, `loadSite`, `loadSitesList`, `mergePulledSites`, `migrateOldAutosave`, `migrateScenarios`, `migrateSiteGroups`, `notifySiteModelChanged`, `onSiteModelChanged`, `pendingLegacyCount`, `pendingLegacySites`, `pruneMigratedLegacy`, `pullCloud`, `purgeDeletedProject`, `purgeExpiredDeletedProjects`, `pushModelToCloud`, `pushSiteToCloud`, `reconcileSiteFromCloud`, `recordSiteTombstone`, `renameSiteGroup`, `repairSplitProjectNames`, `resolveOrCreateTrackedSiteForComp`, `resolveOrCreateTrackedSiteForOverlay`, `restoreDeletedProject`, `saveAutosave`, `saveSite`, `scheduleLinkOf`, `setActiveUser`, `setCurrentSiteId`, `setScheduleLink`, `setSiteGroupRole`, `SITE_TOMB_GRACE_MS`, `siteNameOf`, `snapshotVersion`, `stageLegacySite`, `storage`, `summarizeVersion`
...
- **`public/sequence/index.html`** — Self-contained Sequence/Schedule iframe app: spreadsheet with an Excel-like formula engine, dependency-graph task scheduling, Gantt + PDF export, Supabase cloud-save/history, suggestions review queue, site-linked
  - _exports_: `_cpMeasure`, `_deepClone`, `_formulaBuiltinMap`, `_mEq`, `_mIdArr`, `_mIsObj`, `_mStripRev`, `_pfCostRef`, `_pfDateRef`, `_pfNumOrBlank`, `activeEffectiveCols`, `addBD`, `addCalendarMonths`, `addD`, `addMonths`, `AddNoteComposer`, `addWorkdays`, `agendaDeadline`, `agendaLeadSummary`, `App`, `applyMeetingBinding`, `approxTextPx`, `AutomationPanel`, `avatarColor`, `balOf`, `barLabelText`, `buildFormulaRowColumns`, `buildGanttSVG`, `buildHolidaySet`, `buildPDFHtml`, `cadenceSummary`, `calcEnd`, `cascadeDates`, `celebrateTaskComplete`, `Cell`, `cleanEmailBody`, `colArray`, `colLabel`, `collectCountable`, `collectNonBlank`, `collectNums`, `collectNumsKind`, `collectRefs`, `colLettersToNum`, `colNumToLetters`, `compareValues`, `compareValuesSafe`, `computeDisplayHealth`, `computeFormulaValues`, `computeRolledHealth`, `ConditionGroup`, `ConditionRow`, `constrainedStartFrom`, `ContactPicker`, `ContactsPanel`, `coreChanged`, `cumArgs`, `datedif`, `defaultStartFor`, `DeleteChildrenModal`, `depAnchors`, `DepCell`, `depCellFullTitle`, `DesktopSplitView`, `detectCascadeDrift`, `dif`, `difBD`, `dropFormulaSheetRefs`, `durTooltip`, `effectiveColsOf`, `ErrorBoundary`, `errVal`, `escapeHtml`, `escapeRegExp`, `estLabelWidth`, `evalBinary`, `evalCall`, `evalConditionGroup`, `evalFieldCondition`, `evalHealthCondition`, `evalHealthRules`, `evalNode`, `evalRule`, `evaluateFormula`, `extractColorTag`, `extractRefs`, `extraHolidaySet`, `fd`, `fdLocal`, `ferr`, `Field`, `findSignChange`, `finishCallResult`, `fitCaptionFs`, `flatOrderWithLevel`, `flattenArgs`, `fmtBarDate`, `fmtD`, `fmtPreds`, `fmtTaskDuration`, `formatDateToken`, `formatNumberSection`, `formatNumberToken`, `FormatPanel`, `formatSheetQualifier`, `formatValue`, `formatValueColor`, `formulaCalendar`, `formulaColsOf`, `formulaColToColDef`, `FormulaColumnEditor`, `formulaColumnNames`, `FormulaError`, `formulaToday`, `fvOf`, `ganttNameWeight`, `GanttView`, `getHealthRules`, `getInitials`, `GridColumnsChooser`, `GridHeaderMenu`, `GridView`, `HealthColHeader`, `HealthColHeaderIcon`, `HealthDropMenu`, `HealthPicker`, `HistoryPanel`, `indentSelection`, `InlineDate`, `ipmtOf`, `irrOf`, `isBlank`, `isBuiltinFormulaName`, `isCallInvariant`, `isDate`, `isErrVal`, `isFormulaError`, `isLeap`, `isOrgOwned`, `isoToSerial`, `isoWeekNum`, `isRangeArg`, `isRowInvariant`, `isSiteOwned`, `isValidColor`, `isWorkingSerial`, `layoutExhibitCols`, `loadExportPrefs`, `loadTabView`, `loadViewPrefs`, `looksLikeDateFormat`, `looseEqual`, `makeDate`, `MasterView`, `matchesCriteria`, `matchIndex`, `matchRefToken`, `mbFmt`, `mbOrdinal`, `mbRuleSummary`, `measuredColWidth`, `meetingBoundInfo`, `MeetingCalendarsModal`, `meetingCostDays`, `meetingDatesInRange`, `meetingFloatBD`, `mergeCloudDoc`, `migrateCfRulesToHealthRules`, `migrateRule`, `migrateScheduleOwnership`, `mirrOf`, `mkt`, `moveSelectionToDestination`, `MoveToModal`, `nameCollision`, `need`, `networkDays`, `newMeetingBody`, `nextEligibleMeeting`, `normalizeName`, `normalizeScheduleOwnership`, `normPreds`, `noteISO`, `NoteRow`, `NotesColHeader`, `noteShortMD`, `NotesModal`, `notifyRowsDeleted`, `nperOf`, `nthWeekday`, `nthWeekdayOfMonth`, `nthWeekdayOnOrAfter`, `num1`, `numberFormatColorFor`, `numToGeneralStr`, `opsForField`, `outdentSelection`, `ownerKeyOf`, `ownerOf`, `parse`, `parseCriteriaOperand`, `parseDurationInput`, `parseFlexDate`, `parseFormula`, `parseFormulaUncached`, `parseLooseDate`, `parsePreds`, `parseRefText`, `partitionSchedules`, `pd`, `PDFExportModal`, `persistViewState`, `PhoneSplitView`, `placeGanttLabel`, `planFormulaColumns`, `pmtOf`, `PredEditor`, `predLagPlainEnglish`, `ProjContextMenu`, `ProjDropdown`, `promoteChildrenAndDelete`, `pruneOrphanScheduleRefs`, `pruneScheduleRefs`, `pvOf`, `raiseIfErr`, `rangeArray`, `rangeGrid`, `rateOf`, `readGridCell`, `readGridCfg`, `rebuildHEALTH`, `rebuildMeetingBodyIndex`, `recascadeWithDrift`, `recomputeAfterStructureChange`, `recomputeSchedule`, `reconcileNeedsAttention`, `refToText`, `RenameModal`, `renumberTasks`, `resolveDuration`, `resolveLabelAlign`, `resolveNamedRange`, `resolveSheetGrid`, `resolveTaskSpan`, `resolveViewState`, `rewriteFormulaForCopy`, `rewriteFormulaForSheetRename`, `rewriteFormulaForStructuralShift`, `rollForwardToWorkday`, `rollupParentDates`, `roundAwayFromZero`, `Row`, `RuleColorPicker`, `sameFamily`, `saveExportPrefs`, `saveTabView`, `saveViewPrefs`, `scheduleList`, `schedulesForOwner`, `sectionForSign`, `SegBtn`, `serialToISO`, `serialToYMD`, `SettingsPanel`, `sgqAvatarColor`, `SgqCascadePicker`, `SgqChangeRow`, `sgqChildrenOf`, `sgqConfClass`, `SgqCrumb`, `sgqCrumbOf`, `SgqDot`, `sgqFmtDate`, `sgqInitials`, `sheetQualifierAt`, `showToast`, `solveRoot`, `sortByVisualOrder`, `splitFormatSections`, `SplitView`, `startForEnd`, `StatusHeaderDropdown`, `StatusPicker`, `stepVisibleColIdx`, `stripFormatLiterals`, `stripViewState`, `subBD`, `SuccessorPromptModal`, `SuggestionsView`, `suggestScheduleName`, `TaskContextMenu`, `taskDurUnit`, `taskDurValue`, `textFormat`, `toBool`, `toDateSerial`, `Toggle`, `tokenize`, `toNumber`, `toShortDate`, `toStr`, `touchesSchedule`, `TYPE_RANK`, `validateNewSchedule`, `validatePredEdit`, `valueFamily`, `weekdayOf`, `weekNum`, `wildcardToRegExp`, `withLastActiveSite`, `workdaysBetween`, `writeGridCfg`, `writeLabelAlign`, `xirrOf`, `xnpvPairs`, `yearFrac`, `ymdToSerial`
...
- **`src/workspaces/scheduler/components/NewScheduleModal.jsx`** — TODO — describe
  - _exports_: `default (NewScheduleModal)`
- **`src/workspaces/scheduler/components/ScheduleCrumb.jsx`** — TODO — describe
  - _exports_: `default (ScheduleCrumb)`
- **`src/workspaces/scheduler/components/ScheduleOwnerList.jsx`** — TODO — describe
  - _exports_: `default (ScheduleOwnerList)`
...
- **`src/workspaces/library/lib/folders.js`** — Client folder-index store (B650): Supabase tree CRUD (own-row RLS) + idempotent template seed + the one-way Drive-mirror trigger via /api/folders.
  - _exports_: `addFolder`, `ensureSeeded`, `listFolders`, `migrateAllProjects`, `migrateProjectFiles`, `moveDriveFileToFolder`, `moveFolder`, `planFolderDelete`, `purgeProjectFolders`, `renameFolder`, `syncFoldersToDrive`, `trashSubtree`
