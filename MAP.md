...
- **`src/workspaces/dashboard/lib/compsCardModel.js`** — TODO — describe
  - _exports_: `bandSentenceLabel`, `buildCompsCardData`, `buildPeerSet`, `CARD_LEASE_PERIOD`, `compHeadlineRate`, `compScaleKey`, `compScaleLayout`, `compSizeSf`, `countyEntry`, `countyLabel`, `MIN_PEERS_FOR_SCALE`, `mostRecentlyAddedComp`, `peerComparisonSentence`, `relativeTimeLabel`, `scaleLabel`, `SIZE_BANDS`, `sizeBandFor`, `TYPE_LABEL`
...
- **`src/workspaces/dashboard/lib/pursuitsList.js`** — Pure Pursuits table model: soonest-contractual-date sort, quiet-days fold, tone helpers.
  - _exports_: `isQuietEmphasized`, `NEXT_DATE_FIELDS`, `NEXT_LINE_SOON_DAYS`, `NEXT_LINE_URGENT_DAYS`, `nextContractualDate`, `nextLineTone`, `pursuitsTable`, `QUIET_EMPHASIS_DAYS`, `quietDaysByGroupFromRecency`, `quietDaysByGroupFromRows`
...
- **`src/workspaces/dashboard/lib/sinceLastHereFeed.js`** — TODO — describe
  - _exports_: `buildSinceLastHereFeed`, `capRowsFairlyByKind`, `KIND_META`
...
- **`src/shared/projects/docProjectLiveness.js`** — TODO — describe
  - _exports_: `docProjectIsDead`, `liveProjectIds`
- **`src/shared/projects/projectModel.js`** — Pure project-model helpers: collapse site records into one project per site-group, name-match suggest, dropdown filter, and relative-time formatting for the breadcrumb switcher
  - _exports_: `DELETED_RETENTION_DAYS`, `filterProjects`, `groupProjects`, `markProjectFreshlyMinted`, `normalizeProjectName`, `projectGateStatus`, `relTime`, `resolveControlledId`, `resolveCurrentName`, `suggestNameMatch`, `unionProjectLists`, `wasProjectFreshlyMinted`, `withCurrentProject`
...
- **`src/shared/schedule/scheduleOwnership.js`** — TODO — describe
  - _exports_: `isOrgOwned`, `isSiteOwned`, `migrateScheduleOwnership`, `nameCollision`, `normalizeName`, `normalizeScheduleOwnership`, `ORG_OWNER_KEY`, `ORG_OWNER_LABEL`, `OWNER_KIND_ORG`, `OWNER_KIND_SITE`, `ownerKeyOf`, `ownerOf`, `partitionSchedules`, `pruneOrphanScheduleRefs`, `pruneScheduleRefs`, `scheduleList`, `schedulesForOwner`, `suggestScheduleName`, `validateNewSchedule`
