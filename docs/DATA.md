...
| **A project's name** (a "site group") | **nothing textual** — it is the *majority/stamped* answer computed by `projectName.nameAuthority()` over every plan in the group | every plan's `site` column / `data.site` field (denormalized on purpose, for read paths that only ever see one plan) | `storage.renameSiteGroup` → `cloudSync.cloudRenameGroup` → **one** `rename_site_group()` SQL statement, `UPDATE … WHERE coalesce(data->>'groupId', id) = p_group_id` | `siteRenamedAt` (a stamp, schema v13) — **never `updatedAt`**, which the record shows disagrees with reality on real accounts **⛔ NEW-1 (2026-09-10) — AND THE STAMP HAS EXACTLY ONE WRITER.** The ordinary document push was a second, accidental writer of it: `createSiteModel` normalises an unknown marker to an explicit `siteRenamedAt: null` and `siteRowFor` sends the whole model as `data`, which REPLACES the row's jsonb — so a device whose cache predated a rename wrote that null over the rename's own stamp. Measured on production 2026-09-10: 64 of 116 rows carried a present-but-EMPTY marker (a present null key is a WRITE, not an absence), 34 a real number, 18 no key; Silvestri's plan `sms9c5oc7jnt` had the group's 2026-07-31 stamp erased by a write five days later while its four siblings kept it. **Missing OR empty both read as UNKNOWN, never as "never renamed"** — one parse, `projectName.renameStamp`, and the write path OMITS what it does not know (`normalizeRenameStampForWrite` in `slimForCloud`) rather than asserting null. Because omitting still erases (the write replaces the whole jsonb), the row itself holds the line: `db/sites_rename_stamp_guard.sql` — an UPDATE carrying no real stamp may neither clear a stamped row's marker nor change its project name. |
...
| **An org-scoped agenda item** (B1020930) | `localStorage["planyr:agenda:v1:<scope>"]` — one array of items per account (or `local`, signed out); `scope` is the account id, never a project | none — there is no server copy at all yet | `agendaStore.js`'s `writeAgenda`, called from `AgendaView.jsx` on every add/edit/toggle/delete | **not multi-writer — a deliberate, stated scope cut, not an oversight.** No cloud table, no `rev`, no merge: this is device-local only, the same limit `notesVersions.js`'s version history already carries for the identical reason. Cross-device sync is filed separately (**B1023024**) rather than half-built here. |
| **A site's ROLE** (pursuit vs tracked — B843792, NEW-1) | `data->>'role'` inside the plan's own `public.sites` row (`lib/siteStatus.js`), denormalized across a group's plans exactly like `status` and `site` (name) already are | none | `storage.saveSite` (a single plan) / `storage.setSiteGroupRole` → `set_site_group_role()` SQL statement, `UPDATE … WHERE coalesce(data->>'groupId', id) = p_group_id` (the SAME shape `rename_site_group()` uses, for the same reason: a flip must reach every plan in the group, not just the ones one browser has hydrated) | absent = "pursuit" for every record, legacy or fresh (no majority/stamp rule needed — role never existed before this, so there is no prior population to reconcile against, unlike a project's name). **⛔ B1181104 — a flip through `set_site_group_role()` is a genuine cloud write and MUST stamp `data->'updatedAt'`, not just the outer `updated_at` column.** `mergeSiteContent`'s pull-vs-local merge resolves every scalar (role included) from whichever side's jsonb-INTERNAL `updatedAt` is newer, keeping the LOCAL side on a tie (B559) — a server-side write that bumps only the outer column is invisible to that comparison, so a client with any locally-cached copy can get permanently stuck on the pre-flip role, immune to every later pull. This is exactly why a project's NAME uses its own dedicated `siteRenamedAt` stamp instead of `updatedAt` (see the row below) — role doesn't need a second stamp, it just needs the RPC to keep the ONE stamp `mergeSiteContent` actually reads current, which the original ship of this RPC failed to do. |
| **A comp's owning site** (B843792/B1156864, NEW-1; the runtime mechanism is B1165441, NEW-2/NEW-3) | `comps.project_id` — a real column that already existed and already pointed at `public.sites(id)`, but was NULL on every row until this migration; it is now the load-bearing "which site is this comp about" link, and no second column was added beside it (see `comps.sql`'s own header for why) | `CompsPanel.jsx`'s create/edit form mirrors it as `draft.projectId` | the one-time backfill (`db/site_role_unify_backfill_20260905.sql`) judged the three comps that existed the day it ran; **every comp saved since goes through the runtime mechanism** — `CompsPanel.jsx`'s `save()` calls `storage.resolveOrCreateTrackedSiteForComp`, which runs `compSiteMatch.findMatchingSite` (0.5mi location match against ANY existing site of any role, else an exact normalized-title match) against `loadSitesList()`, attaching to a match or minting a new role-"tracked" site — or the existing per-comp edit-form dropdown (manual reassign) | not multi-writer (comps are owner-only write per `comps.sql`'s RLS); a comp with no owning site cannot exist going forward — `save()` never lets `insertComp`/`updateComp` run with `project_id` still null when an anchor is present |
...
| "Is this `siteRenamedAt` a real stamp, or UNKNOWN?" | `projectName.renameStamp(v)` (+ `normalizeRenameStampForWrite` for the write side; `public.rename_stamp(jsonb)` is its SQL mirror) | `test/renameStampIntegrity.test.js` sweeps every declared reader for a hand-rolled `typeof … === "number"` / `isFinite(` / `Number(` test applied to the marker and fails on any that does not route through `renameStamp` — four modules had independently re-implemented it, which is how "unknown" and "never renamed" became two answers to one question |
