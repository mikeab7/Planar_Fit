...
|---|---|---|---|---|---|
| Map landing page (no project selected) | 20 | 20 | 18 | 5 | 23 |
| Map landing page (decide bar) | 24 | 24 | 18 | 6 | 24 |
| Map landing page (selecting parcels) | 20 | 20 | 18 | 5 | 23 |
...
- **B1401952-no-location-flag** (2026-09-09) — +1 on "Map landing page (decide bar)": Owner report — the Dashboard Locations map card's 'N projects' location(s) need fixing' link used to land on the Site Planner's plain, unfiltered project list with nothing marking which projects it meant. The Sites panel row now carries a small 'no location' flag next to any site with no origin (src/workspaces/site-planner/MapFinder.jsx's `rowFlagBadgeStyle`), the same standing-fact badge shape the existing 'no boundary' flag already uses on this same row (identical radius/height/padding/fontSize AND color — one shared style object, not a bespoke variant) — but that sibling flag's own precondition (a site with a boundary field but no drawn boundary) is never true in this crawl's fixture, so it has never itself been measured as a signature on this surface, and this is the first member of that shared shape this particular crawl has ever counted here. A code fix cannot remove this signature without either dropping the flag the owner's report asked for, or resizing it to coincidentally match an unrelated control already on the surface, which would be the wrong reason to change either one.

- **B842864-global-help-report-control** (2026-09-05) — +1 on "Map landing page (no project selected)", "Map landing page (decide bar)", "Map landing page (selecting parcels)", "Left rail + panels (Yield)", "Library", "Doc Review (empty state)": The global help/report control (src/app/HelpReportControl.jsx) is a persistent, fixed bottom-right FAB the app shell mounts once, on every route — the owner's own request, so the control being everywhere is the point, not drift. It renders one uniform signature (radius:8px/RADIUS.md since B1176976, padding:0px, fontSize:12px) on every whole-page surface this crawl visits — height is 30px (CONTROL_H.lg) under this crawl's fine-pointer, no-touch context since B1162016, 44px only under a coarse/touch pointer, never both at once; the surfaces that scope to a sub-region only (App header, the Main menu popovers, Tool rail) don't see it because it lives outside that DOM subtree, which is why only the whole-page surfaces above needed a budget lift. A single shared implementation, one exemption line per affected surface, never a per-surface variant.

- **B1038016-leaflet-chrome** (2026-09-01) — +3 on "Map landing page (no project selected)", "Map landing page (decide bar)", "Map landing page (selecting parcels)": Leaflet's own bundled zoom stack (+/− at its vendor font-size), "Find my location" locate button, and the scale bar are third-party chrome — docs/DESIGN.md's radius section (Documented exceptions #4) already carries the zoom-stack/locate-button radius override and the scale-bar non-exception as the accepted boundary of what this token scale reaches. Their child <a>/<div> nodes are deliberately radius:0 BY THE APP'S OWN CSS (the rounded corner the user sees comes from the parent .leaflet-bar container's overflow:hidden clip, not the button), so a per-button override here would fight the exact mechanism that already unified their visible shape with the app's own RADIUS.md. The one thing left off-scale (the zoom glyphs' 22px font) is Leaflet's own vendor CSS (.leaflet-touch .leaflet-control-zoom-in/-out), not an app literal.
...
- "Search for an address or place" sits 17px from "Click parcels on the map to select them, then say what they are" — height 42px vs 30px, padding "0px 10px" vs "0px 12px" (divider-separated).
...
**Map landing page (decide bar) — light:**

- "Settings" sits 6px from "Sign in or create an account" — padding "0px" vs "0px 12px".
- "Search for an address or place" sits 188px from "Remove the pin" — height 42px vs 26px, padding "0px 10px" vs "1px 6px" (divider-separated).
- "Remove the pin" sits 0px from "Start a plan on this ground" — height 26px vs 30px, padding "1px 6px" vs "0px 12px".
- "Collapse the sites panel" sits 4px from "Sites1" — padding "1px 6px" vs "0px 8px".
- "Collapse" sits 0px from "Reorder the Pursuit group" — height 23px vs 17px, padding "4px 4px 4px 12px" vs "1px 6px".

**Map landing page (decide bar) — dark:**
...
- Select parcelsDrawDrop a pin (top 67, h 42) · ▼Sites1Comps0LargestA–ZRecent▼ (top 67, h 128) · ▼Imagery & layers (top 67, h 30) — top spread 0px, height spread 98px.
...
- Pin dropped✕Plan a siteLog a c (top 67, h 42) · ▼Sites1Comps0LargestA–ZRecent▼ (top 67, h 128) · ▼Imagery & layers (top 67, h 30) — top spread 0px, height spread 98px.
...
- Selecting…Drop a pinCancel (top 67, h 42) · ▼Sites1Comps0LargestA–ZRecent▼ (top 67, h 128) · ▼Imagery & layers (top 67, h 30) — top spread 0px, height spread 98px.
...
|---|---|---|---|---|---|---|---|
| ⚠️ **fontSize 22 not in FONT_SIZE scale {10,10.5,12,13,14}** — 0px | 30px | 22px | 700 | `rgb(255, 255, 255)` | `0px none rgb(27, 30, 38)` | Zoom in, Zoom out ×2 | src/shared/sitePlans/components/SitePlansSection.jsx:391:          <Button size="sm" variant="ghost" onClick={() => onZoomToOverlay(o)}>Zoom in</Button> (+more matches elsewhere, best-effort) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 42px | 16px | 400 | `rgb(234, 238, 243)` | `0px none rgb(27, 30, 38)` | Select parcelsDrawDrop a pin ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 128px | 16px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | ▼Sites1Comps0LargestA–ZRecent▼Pursuit1UI ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 9.5 not in FONT_SIZE scale {10,10.5,12,13,14}** — 999px | 15px | 9.5px | 700 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | No location set — open this project, then Land → Set this plan's location ×1 | src/workspaces/site-planner/MapFinder.jsx:3717:                <span title="No location set — open this project, then Land → Set this plan's location" style={rowFlagBadgeStyle}>no location</span> |
...
| 6px | 30px | 12px | 600 | `rgb(255, 255, 255)` | `1px solid rgb(215, 221, 229)` | Start a plan with no parcel, where the map is looking — draw the boundary yourself, Click anywhere on the map to mark a point, then say what it is ×2 | src/workspaces/site-planner/MapFinder.jsx:4018:                title="Start a plan with no parcel, where the map is looking — draw the boundary yourself" |
...
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(53, 59, 73)` | Choose a project or organization ×1 | src/shared/ui/ProjectBreadcrumb.jsx:802:        title={cross ? "Browsing all projects" : org ? "Browsing your organization's notes, library and agenda" : currentProject ? "Switch project" : "Choose a project or organization"} |
...
| 0px | 30px | 12px | 400 | `rgb(255, 255, 255)` | `0px none rgb(27, 30, 38)` | Find my location ×1 | src/workspaces/site-planner/MapFinder.jsx:1590:      btn.href = "#"; btn.setAttribute("role", "button"); btn.setAttribute("aria-label", "Find my location"); btn.setAttribute("data-testid", "locate-me-btn"); btn.setAttribute("data-locate-state", "idle"); (+more matches elsewhere, best-effort) |
| 8px | 18px | 12px | 400 | `rgb(255, 255, 255)` | `0px none rgb(225, 229, 235)` | 500 mi ×1 | unattributed (no source match — best-effort text search) |
| 8px | 30px | 12px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(205, 211, 220)` | Help and report a problem ×1 | src/app/HelpReportControl.jsx:359:      aria-label="Help and report a problem" |
...
| 6px | 30px | 12px | 700 | `rgb(194, 65, 12)` | `1px solid rgb(194, 65, 12)` | Click parcels on the map to select them, then say what they are ×1 | src/workspaces/site-planner/MapFinder.jsx:4008:                title="Click parcels on the map to select them, then say what they are" |
| 6px | 22px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(53, 59, 73)` | Collapse the sites panel ×1 | src/workspaces/site-planner/MapFinder.jsx:4272:                title={sitesPanelOpen ? "Collapse the sites panel" : "Expand the sites panel"} |
...
| 6px | 29px | 12px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | Filter sites by name ×1 | src/workspaces/site-planner/MapFinder.jsx:4315:              <input value={nameFilter} onChange={(e) => setNameFilter(e.target.value)} placeholder="Filter by name…" aria-label="Filter sites by name" (+more matches elsewhere, best-effort) |
| 6px | 29px | 12px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | Sort sites within each group ×1 | src/workspaces/site-planner/MapFinder.jsx:4319:                  "Filter sites by name sits 6px from Sort sites within each group — padding (+more matches elsewhere, best-effort) |
| 0px | 23px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(0, 0, 0)` | Collapse ×1 | src/shared/comps/components/CompEntryGrid.jsx:1893:          <table style={{ borderCollapse: "collapse", tableLayout: "fixed" }}> (+more matches elsewhere, best-effort) |
...
| 8px | 28px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(27, 30, 38)` | Imagery & layers ×1 | src/app/HelpReportControl.jsx:44: * the screen, tucked under "Imagery & layers", nowhere near the corner he asked for. Fixed by (+more matches elsewhere, best-effort) |

**dark** — 28 distinct style signature(s) over 35 matched element(s):

| radius | height | font | weight | background | border | label(s) | file/line (best-effort) |
|---|---|---|---|---|---|---|---|
| ⚠️ **fontSize 22 not in FONT_SIZE scale {10,10.5,12,13,14}** — 0px | 30px | 22px | 700 | `rgb(24, 27, 33)` | `0px none rgb(236, 239, 244)` | Zoom in, Zoom out ×2 | src/shared/sitePlans/components/SitePlansSection.jsx:391:          <Button size="sm" variant="ghost" onClick={() => onZoomToOverlay(o)}>Zoom in</Button> (+more matches elsewhere, best-effort) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 42px | 16px | 400 | `rgb(17, 19, 25)` | `0px none rgb(232, 235, 240)` | Select parcelsDrawDrop a pin ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 128px | 16px | 400 | `rgb(24, 27, 33)` | `1px solid rgb(42, 46, 55)` | ▼Sites1Comps0LargestA–ZRecent▼Pursuit1UI ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 9.5 not in FONT_SIZE scale {10,10.5,12,13,14}** — 999px | 15px | 9.5px | 700 | `rgb(24, 27, 33)` | `1px solid rgb(42, 46, 55)` | No location set — open this project, then Land → Set this plan's location ×1 | src/workspaces/site-planner/MapFinder.jsx:3717:                <span title="No location set — open this project, then Land → Set this plan's location" style={rowFlagBadgeStyle}>no location</span> |
...
| 6px | 30px | 12px | 600 | `rgb(23, 26, 33)` | `1px solid rgb(38, 42, 51)` | Start a plan with no parcel, where the map is looking — draw the boundary yourself, Click anywhere on the map to mark a point, then say what it is ×2 | src/workspaces/site-planner/MapFinder.jsx:4018:                title="Start a plan with no parcel, where the map is looking — draw the boundary yourself" |
| 6px | 23px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(0, 0, 0)` | Dashboard: all projects ×1 | src/shared/ui/dashboardNav.js:27:    title: action ? (logoDashboardTitle || "Dashboard: all projects") : undefined, |
...
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(166, 173, 186)` | Choose a project or organization ×1 | src/shared/ui/ProjectBreadcrumb.jsx:802:        title={cross ? "Browsing all projects" : org ? "Browsing your organization's notes, library and agenda" : currentProject ? "Switch project" : "Choose a project or organization"} |
...
| 0px | 30px | 12px | 400 | `rgb(24, 27, 33)` | `0px none rgb(236, 239, 244)` | Find my location ×1 | src/workspaces/site-planner/MapFinder.jsx:1590:      btn.href = "#"; btn.setAttribute("role", "button"); btn.setAttribute("aria-label", "Find my location"); btn.setAttribute("data-testid", "locate-me-btn"); btn.setAttribute("data-locate-state", "idle"); (+more matches elsewhere, best-effort) |
| 8px | 18px | 12px | 400 | `rgb(24, 27, 33)` | `0px none rgb(42, 46, 55)` | 500 mi ×1 | unattributed (no source match — best-effort text search) |
| 8px | 30px | 12px | 400 | `rgb(29, 32, 39)` | `1px solid rgb(58, 63, 75)` | Help and report a problem ×1 | src/app/HelpReportControl.jsx:359:      aria-label="Help and report a problem" |
...
| 6px | 30px | 12px | 700 | `rgb(242, 107, 58)` | `1px solid rgb(242, 107, 58)` | Click parcels on the map to select them, then say what they are ×1 | src/workspaces/site-planner/MapFinder.jsx:4008:                title="Click parcels on the map to select them, then say what they are" |
| 6px | 22px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(202, 208, 218)` | Collapse the sites panel ×1 | src/workspaces/site-planner/MapFinder.jsx:4272:                title={sitesPanelOpen ? "Collapse the sites panel" : "Expand the sites panel"} |
...
| 6px | 29px | 12px | 400 | `rgb(29, 32, 39)` | `1px solid rgb(42, 46, 55)` | Filter sites by name ×1 | src/workspaces/site-planner/MapFinder.jsx:4315:              <input value={nameFilter} onChange={(e) => setNameFilter(e.target.value)} placeholder="Filter by name…" aria-label="Filter sites by name" (+more matches elsewhere, best-effort) |
| 6px | 29px | 12px | 400 | `rgb(29, 32, 39)` | `1px solid rgb(42, 46, 55)` | Sort sites within each group ×1 | src/workspaces/site-planner/MapFinder.jsx:4319:                  "Filter sites by name sits 6px from Sort sites within each group — padding (+more matches elsewhere, best-effort) |
| 0px | 23px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(0, 0, 0)` | Collapse ×1 | src/shared/comps/components/CompEntryGrid.jsx:1893:          <table style={{ borderCollapse: "collapse", tableLayout: "fixed" }}> (+more matches elsewhere, best-effort) |
| 999px | 14px | 10px | 400 | `rgb(216, 90, 48)` | `0px none rgb(255, 255, 255)` | SPAN ×1 | src/shared/files/matchLineFit.js:131:  // Least-squares refit on inliers for sub-pixel accuracy; also track how far the inliers SPAN (+more matches elsewhere, best-effort) |
...
| 8px | 28px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(232, 235, 240)` | Imagery & layers ×1 | src/app/HelpReportControl.jsx:44: * the screen, tucked under "Imagery & layers", nowhere near the corner he asked for. Fixed by (+more matches elsewhere, best-effort) |


---

### Map landing page (decide bar)

**light** — 32 distinct style signature(s) over 40 matched element(s):

| radius | height | font | weight | background | border | label(s) | file/line (best-effort) |
|---|---|---|---|---|---|---|---|
| ⚠️ **fontSize 22 not in FONT_SIZE scale {10,10.5,12,13,14}** — 0px | 30px | 22px | 700 | `rgb(255, 255, 255)` | `0px none rgb(27, 30, 38)` | Zoom in, Zoom out ×2 | src/shared/sitePlans/components/SitePlansSection.jsx:391:          <Button size="sm" variant="ghost" onClick={() => onZoomToOverlay(o)}>Zoom in</Button> (+more matches elsewhere, best-effort) |
| ⚠️ **fontSize 11 not in FONT_SIZE scale {10,10.5,12,13,14}** — 8px | 21px | 11px | 400 | `rgba(0, 0, 0, 0.5)` | `0px none rgba(255, 255, 255, 0.9)` | Ground elevation at the cursor — USGS 3DEP LiDAR bare-earth, NAVD88. Screening only — verify with survey. The elevation source reports no data at this point (open water or a LiDAR void). ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 42px | 16px | 400 | `rgb(234, 238, 243)` | `0px none rgb(27, 30, 38)` | Pin dropped✕Plan a siteLog a compPlace a ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 999px | 7px | 16px | 400 | `rgb(53, 59, 73)` | `0px none rgb(27, 30, 38)` | map-decide-dot ×1 | src/workspaces/site-planner/MapFinder.jsx:4121:              <span data-testid="map-decide-dot" style={{ width: 7, height: 7, borderRadius: RADIUS.pill, background: PAL.chromeMuted, flex: "none" }} /> |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 128px | 16px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | ▼Sites1Comps0LargestA–ZRecent▼Pursuit1UI ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 9.5 not in FONT_SIZE scale {10,10.5,12,13,14}** — 999px | 15px | 9.5px | 700 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | No location set — open this project, then Land → Set this plan's location ×1 | src/workspaces/site-planner/MapFinder.jsx:3717:                <span title="No location set — open this project, then Land → Set this plan's location" style={rowFlagBadgeStyle}>no location</span> |
| 0px | 25px | 12px | 500 | `rgba(0, 0, 0, 0)` | `0px none rgb(69, 76, 92)` | Schedule, Review, Library (+2 more) ×5 | src/app/AccountControl.jsx:7: * once (Site map header + Site plan header + DocReview + Library + Scheduler), most of them (+more matches elsewhere, best-effort) |
| 6px | 30px | 12px | 600 | `rgb(255, 255, 255)` | `1px solid rgb(215, 221, 229)` | Record this as a leasing or sale comp, Upload a broker flyer or park plan and place it on this ground, Pin a note to this ground ×3 | src/workspaces/site-planner/MapFinder.jsx:3423:      title: "Record this as a leasing or sale comp", |
| 8px | 30px | 12px | 400 | `rgb(234, 238, 243)` | `1px solid rgb(215, 221, 229)` | Full screen, Settings ×2 | src/shared/ui/AppHeader.jsx:153:      aria-label={active ? "Leave full screen" : "Full screen"} (+more matches elsewhere, best-effort) |
| 6px | 23px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(0, 0, 0)` | Dashboard: all projects ×1 | src/shared/ui/dashboardNav.js:27:    title: action ? (logoDashboardTitle || "Dashboard: all projects") : undefined, |
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(27, 30, 38)` | All projects: Map ×1 | unattributed (no source match — best-effort text search) |
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(53, 59, 73)` | Choose a project or organization ×1 | src/shared/ui/ProjectBreadcrumb.jsx:802:        title={cross ? "Browsing all projects" : org ? "Browsing your organization's notes, library and agenda" : currentProject ? "Switch project" : "Choose a project or organization"} |
...
| 999px | 20px | 10.5px | 800 | `rgb(234, 238, 243)` | `0px none rgb(27, 30, 38)` | › ×1 | unattributed (label too short to search) |
| 0px | 25px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(15, 110, 86)` | Site ×1 | src/app/AccountControl.jsx:7: * once (Site map header + Site plan header + DocReview + Library + Scheduler), most of them (+more matches elsewhere, best-effort) |
| 999px | 14px | 12px | 400 | `rgb(194, 65, 12)` | `2px solid rgb(255, 255, 255)` | map-decide-pin ×1 | src/workspaces/site-planner/MapFinder.jsx:3553:    const html = `<div data-testid="map-decide-pin" style="` |
| 0px | 30px | 12px | 400 | `rgb(255, 255, 255)` | `0px none rgb(27, 30, 38)` | Find my location ×1 | src/workspaces/site-planner/MapFinder.jsx:1590:      btn.href = "#"; btn.setAttribute("role", "button"); btn.setAttribute("aria-label", "Find my location"); btn.setAttribute("data-testid", "locate-me-btn"); btn.setAttribute("data-locate-state", "idle"); (+more matches elsewhere, best-effort) |
| 8px | 18px | 12px | 400 | `rgb(255, 255, 255)` | `0px none rgb(225, 229, 235)` | 500 mi ×1 | unattributed (no source match — best-effort text search) |
| 8px | 30px | 12px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(205, 211, 220)` | Help and report a problem ×1 | src/app/HelpReportControl.jsx:359:      aria-label="Help and report a problem" |
| 0px | 42px | 13px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(27, 30, 38)` | Search for an address or place ×1 | src/workspaces/site-planner/components/PlaceSearchField.jsx:148:        aria-label="Search for an address or place" |
| 6px | 26px | 13px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(53, 59, 73)` | Remove the pin ×1 | src/workspaces/site-planner/MapFinder.jsx:4132:                title={decideTarget === "parcels" ? "Clear selection" : "Remove the pin"} |
| 6px | 30px | 12px | 700 | `rgb(194, 65, 12)` | `0px none rgb(255, 255, 255)` | Start a plan on this ground ×1 | src/workspaces/site-planner/MapFinder.jsx:3407:      title: "Start a plan on this ground", |
| 6px | 22px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(53, 59, 73)` | Collapse the sites panel ×1 | src/workspaces/site-planner/MapFinder.jsx:4272:                title={sitesPanelOpen ? "Collapse the sites panel" : "Expand the sites panel"} |
| 6px | 22px | 12px | 700 | `rgb(255, 255, 255)` | `0px none rgb(27, 30, 38)` | Sites1 ×1 | unattributed (no source match — best-effort text search) |
| 6px | 22px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(53, 59, 73)` | Comps0 ×1 | unattributed (no source match — best-effort text search) |
| 6px | 29px | 12px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | Filter sites by name ×1 | src/workspaces/site-planner/MapFinder.jsx:4315:              <input value={nameFilter} onChange={(e) => setNameFilter(e.target.value)} placeholder="Filter by name…" aria-label="Filter sites by name" (+more matches elsewhere, best-effort) |
...
**dark** — 32 distinct style signature(s) over 40 matched element(s):

| radius | height | font | weight | background | border | label(s) | file/line (best-effort) |
|---|---|---|---|---|---|---|---|
| ⚠️ **fontSize 22 not in FONT_SIZE scale {10,10.5,12,13,14}** — 0px | 30px | 22px | 700 | `rgb(24, 27, 33)` | `0px none rgb(236, 239, 244)` | Zoom in, Zoom out ×2 | src/shared/sitePlans/components/SitePlansSection.jsx:391:          <Button size="sm" variant="ghost" onClick={() => onZoomToOverlay(o)}>Zoom in</Button> (+more matches elsewhere, best-effort) |
| ⚠️ **fontSize 11 not in FONT_SIZE scale {10,10.5,12,13,14}** — 8px | 21px | 11px | 400 | `rgba(0, 0, 0, 0.5)` | `0px none rgba(255, 255, 255, 0.9)` | Ground elevation at the cursor — USGS 3DEP LiDAR bare-earth, NAVD88. Screening only — verify with survey. The elevation source reports no data at this point (open water or a LiDAR void). ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 42px | 16px | 400 | `rgb(17, 19, 25)` | `0px none rgb(232, 235, 240)` | Pin dropped✕Plan a siteLog a compPlace a ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 999px | 7px | 16px | 400 | `rgb(166, 173, 186)` | `0px none rgb(232, 235, 240)` | map-decide-dot ×1 | src/workspaces/site-planner/MapFinder.jsx:4121:              <span data-testid="map-decide-dot" style={{ width: 7, height: 7, borderRadius: RADIUS.pill, background: PAL.chromeMuted, flex: "none" }} /> |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 128px | 16px | 400 | `rgb(24, 27, 33)` | `1px solid rgb(42, 46, 55)` | ▼Sites1Comps0LargestA–ZRecent▼Pursuit1UI ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 9.5 not in FONT_SIZE scale {10,10.5,12,13,14}** — 999px | 15px | 9.5px | 700 | `rgb(24, 27, 33)` | `1px solid rgb(42, 46, 55)` | No location set — open this project, then Land → Set this plan's location ×1 | src/workspaces/site-planner/MapFinder.jsx:3717:                <span title="No location set — open this project, then Land → Set this plan's location" style={rowFlagBadgeStyle}>no location</span> |
| 0px | 25px | 12px | 500 | `rgba(0, 0, 0, 0)` | `0px none rgb(194, 200, 210)` | Schedule, Review, Library (+2 more) ×5 | src/app/AccountControl.jsx:7: * once (Site map header + Site plan header + DocReview + Library + Scheduler), most of them (+more matches elsewhere, best-effort) |
| 6px | 30px | 12px | 600 | `rgb(23, 26, 33)` | `1px solid rgb(38, 42, 51)` | Record this as a leasing or sale comp, Upload a broker flyer or park plan and place it on this ground, Pin a note to this ground ×3 | src/workspaces/site-planner/MapFinder.jsx:3423:      title: "Record this as a leasing or sale comp", |
| 8px | 30px | 12px | 400 | `rgb(17, 19, 25)` | `1px solid rgb(38, 42, 51)` | Full screen, Settings ×2 | src/shared/ui/AppHeader.jsx:153:      aria-label={active ? "Leave full screen" : "Full screen"} (+more matches elsewhere, best-effort) |
| 6px | 23px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(0, 0, 0)` | Dashboard: all projects ×1 | src/shared/ui/dashboardNav.js:27:    title: action ? (logoDashboardTitle || "Dashboard: all projects") : undefined, |
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(236, 239, 244)` | All projects: Map ×1 | unattributed (no source match — best-effort text search) |
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(166, 173, 186)` | Choose a project or organization ×1 | src/shared/ui/ProjectBreadcrumb.jsx:802:        title={cross ? "Browsing all projects" : org ? "Browsing your organization's notes, library and agenda" : currentProject ? "Switch project" : "Choose a project or organization"} |
...
| 999px | 20px | 10.5px | 800 | `rgb(17, 19, 25)` | `0px none rgb(236, 239, 244)` | › ×1 | unattributed (label too short to search) |
| 0px | 25px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(93, 202, 165)` | Site ×1 | src/app/AccountControl.jsx:7: * once (Site map header + Site plan header + DocReview + Library + Scheduler), most of them (+more matches elsewhere, best-effort) |
| 999px | 14px | 12px | 400 | `rgb(242, 107, 58)` | `2px solid rgb(255, 255, 255)` | map-decide-pin ×1 | src/workspaces/site-planner/MapFinder.jsx:3553:    const html = `<div data-testid="map-decide-pin" style="` |
| 0px | 30px | 12px | 400 | `rgb(24, 27, 33)` | `0px none rgb(236, 239, 244)` | Find my location ×1 | src/workspaces/site-planner/MapFinder.jsx:1590:      btn.href = "#"; btn.setAttribute("role", "button"); btn.setAttribute("aria-label", "Find my location"); btn.setAttribute("data-testid", "locate-me-btn"); btn.setAttribute("data-locate-state", "idle"); (+more matches elsewhere, best-effort) |
| 8px | 18px | 12px | 400 | `rgb(24, 27, 33)` | `0px none rgb(42, 46, 55)` | 500 mi ×1 | unattributed (no source match — best-effort text search) |
| 8px | 30px | 12px | 400 | `rgb(29, 32, 39)` | `1px solid rgb(58, 63, 75)` | Help and report a problem ×1 | src/app/HelpReportControl.jsx:359:      aria-label="Help and report a problem" |
| 0px | 42px | 13px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(236, 239, 244)` | Search for an address or place ×1 | src/workspaces/site-planner/components/PlaceSearchField.jsx:148:        aria-label="Search for an address or place" |
| 6px | 26px | 13px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(166, 173, 186)` | Remove the pin ×1 | src/workspaces/site-planner/MapFinder.jsx:4132:                title={decideTarget === "parcels" ? "Clear selection" : "Remove the pin"} |
| 6px | 30px | 12px | 700 | `rgb(242, 107, 58)` | `0px none rgb(21, 23, 28)` | Start a plan on this ground ×1 | src/workspaces/site-planner/MapFinder.jsx:3407:      title: "Start a plan on this ground", |
| 6px | 22px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(202, 208, 218)` | Collapse the sites panel ×1 | src/workspaces/site-planner/MapFinder.jsx:4272:                title={sitesPanelOpen ? "Collapse the sites panel" : "Expand the sites panel"} |
| 6px | 22px | 12px | 700 | `rgb(29, 32, 39)` | `0px none rgb(232, 235, 240)` | Sites1 ×1 | unattributed (no source match — best-effort text search) |
| 6px | 22px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(202, 208, 218)` | Comps0 ×1 | unattributed (no source match — best-effort text search) |
| 6px | 29px | 12px | 400 | `rgb(29, 32, 39)` | `1px solid rgb(42, 46, 55)` | Filter sites by name ×1 | src/workspaces/site-planner/MapFinder.jsx:4315:              <input value={nameFilter} onChange={(e) => setNameFilter(e.target.value)} placeholder="Filter by name…" aria-label="Filter sites by name" (+more matches elsewhere, best-effort) |
...
|---|---|---|---|---|---|---|---|
| ⚠️ **fontSize 22 not in FONT_SIZE scale {10,10.5,12,13,14}** — 0px | 30px | 22px | 700 | `rgb(255, 255, 255)` | `0px none rgb(27, 30, 38)` | Zoom in, Zoom out ×2 | src/shared/sitePlans/components/SitePlansSection.jsx:391:          <Button size="sm" variant="ghost" onClick={() => onZoomToOverlay(o)}>Zoom in</Button> (+more matches elsewhere, best-effort) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 42px | 16px | 400 | `rgb(234, 238, 243)` | `0px none rgb(27, 30, 38)` | Selecting…Drop a pinCancel ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 128px | 16px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | ▼Sites1Comps0LargestA–ZRecent▼Pursuit1UI ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 9.5 not in FONT_SIZE scale {10,10.5,12,13,14}** — 999px | 15px | 9.5px | 700 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | No location set — open this project, then Land → Set this plan's location ×1 | src/workspaces/site-planner/MapFinder.jsx:3717:                <span title="No location set — open this project, then Land → Set this plan's location" style={rowFlagBadgeStyle}>no location</span> |
| 0px | 25px | 12px | 500 | `rgba(0, 0, 0, 0)` | `0px none rgb(69, 76, 92)` | Schedule, Review, Library (+2 more) ×5 | src/app/AccountControl.jsx:7: * once (Site map header + Site plan header + DocReview + Library + Scheduler), most of them (+more matches elsewhere, best-effort) |
| 8px | 30px | 12px | 400 | `rgb(234, 238, 243)` | `1px solid rgb(215, 221, 229)` | Full screen, Settings ×2 | src/shared/ui/AppHeader.jsx:153:      aria-label={active ? "Leave full screen" : "Full screen"} (+more matches elsewhere, best-effort) |
| 6px | 30px | 12px | 600 | `rgb(255, 255, 255)` | `1px solid rgb(215, 221, 229)` | Drop a pin instead of anchoring to a parcel, Cancel ×2 | src/workspaces/site-planner/MapFinder.jsx:4096:                  title="Drop a pin instead of anchoring to a parcel" |
| 6px | 23px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(0, 0, 0)` | Dashboard: all projects ×1 | src/shared/ui/dashboardNav.js:27:    title: action ? (logoDashboardTitle || "Dashboard: all projects") : undefined, |
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(27, 30, 38)` | All projects: Map ×1 | unattributed (no source match — best-effort text search) |
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(53, 59, 73)` | Choose a project or organization ×1 | src/shared/ui/ProjectBreadcrumb.jsx:802:        title={cross ? "Browsing all projects" : org ? "Browsing your organization's notes, library and agenda" : currentProject ? "Switch project" : "Choose a project or organization"} |
| 8px | 30px | 12px | 600 | `rgb(255, 255, 255)` | `1px solid rgb(215, 221, 229)` | Sign in or create an account ×1 | src/app/AccountControl.jsx:226:      <MenuTrigger onClick={onOpenAuth} title="Sign in or create an account" caret={false} leading={<span style={avatar(false)}>›</span>} data-testid="account-signed-out"> |
...
| 8px | 30px | 12px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | ▼Imagery & layers ×1 | unattributed (no source match — best-effort text search) |
| 8px | 28px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(27, 30, 38)` | Imagery & layers ×1 | src/app/HelpReportControl.jsx:44: * the screen, tucked under "Imagery & layers", nowhere near the corner he asked for. Fixed by (+more matches elsewhere, best-effort) |
| 12px | 48px | 12px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | Click any lot on the map to add it (＋) — ×1 | src/workspaces/site-planner/MapFinder.jsx:4765:                ? "Click any lot on the map to add it (＋) — it works even before the purple outlines appear. Zoom in a little to see the lines." |
...
|---|---|---|---|---|---|---|---|
| ⚠️ **fontSize 22 not in FONT_SIZE scale {10,10.5,12,13,14}** — 0px | 30px | 22px | 700 | `rgb(24, 27, 33)` | `0px none rgb(236, 239, 244)` | Zoom in, Zoom out ×2 | src/shared/sitePlans/components/SitePlansSection.jsx:391:          <Button size="sm" variant="ghost" onClick={() => onZoomToOverlay(o)}>Zoom in</Button> (+more matches elsewhere, best-effort) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 42px | 16px | 400 | `rgb(17, 19, 25)` | `0px none rgb(232, 235, 240)` | Selecting…Drop a pinCancel ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 16 not in FONT_SIZE scale {10,10.5,12,13,14}** — 12px | 128px | 16px | 400 | `rgb(24, 27, 33)` | `1px solid rgb(42, 46, 55)` | ▼Sites1Comps0LargestA–ZRecent▼Pursuit1UI ×1 | unattributed (no source match — best-effort text search) |
| ⚠️ **fontSize 9.5 not in FONT_SIZE scale {10,10.5,12,13,14}** — 999px | 15px | 9.5px | 700 | `rgb(24, 27, 33)` | `1px solid rgb(42, 46, 55)` | No location set — open this project, then Land → Set this plan's location ×1 | src/workspaces/site-planner/MapFinder.jsx:3717:                <span title="No location set — open this project, then Land → Set this plan's location" style={rowFlagBadgeStyle}>no location</span> |
| 0px | 25px | 12px | 500 | `rgba(0, 0, 0, 0)` | `0px none rgb(194, 200, 210)` | Schedule, Review, Library (+2 more) ×5 | src/app/AccountControl.jsx:7: * once (Site map header + Site plan header + DocReview + Library + Scheduler), most of them (+more matches elsewhere, best-effort) |
| 8px | 30px | 12px | 400 | `rgb(17, 19, 25)` | `1px solid rgb(38, 42, 51)` | Full screen, Settings ×2 | src/shared/ui/AppHeader.jsx:153:      aria-label={active ? "Leave full screen" : "Full screen"} (+more matches elsewhere, best-effort) |
| 6px | 30px | 12px | 600 | `rgb(23, 26, 33)` | `1px solid rgb(38, 42, 51)` | Drop a pin instead of anchoring to a parcel, Cancel ×2 | src/workspaces/site-planner/MapFinder.jsx:4096:                  title="Drop a pin instead of anchoring to a parcel" |
| 6px | 23px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(0, 0, 0)` | Dashboard: all projects ×1 | src/shared/ui/dashboardNav.js:27:    title: action ? (logoDashboardTitle || "Dashboard: all projects") : undefined, |
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(236, 239, 244)` | All projects: Map ×1 | unattributed (no source match — best-effort text search) |
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(166, 173, 186)` | Choose a project or organization ×1 | src/shared/ui/ProjectBreadcrumb.jsx:802:        title={cross ? "Browsing all projects" : org ? "Browsing your organization's notes, library and agenda" : currentProject ? "Switch project" : "Choose a project or organization"} |
| 8px | 30px | 12px | 600 | `rgb(23, 26, 33)` | `1px solid rgb(38, 42, 51)` | Sign in or create an account ×1 | src/app/AccountControl.jsx:226:      <MenuTrigger onClick={onOpenAuth} title="Sign in or create an account" caret={false} leading={<span style={avatar(false)}>›</span>} data-testid="account-signed-out"> |
...
| 8px | 30px | 12px | 400 | `rgb(29, 32, 39)` | `1px solid rgb(42, 46, 55)` | ▼Imagery & layers ×1 | unattributed (no source match — best-effort text search) |
| 8px | 28px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(232, 235, 240)` | Imagery & layers ×1 | src/app/HelpReportControl.jsx:44: * the screen, tucked under "Imagery & layers", nowhere near the corner he asked for. Fixed by (+more matches elsewhere, best-effort) |
| 12px | 48px | 12px | 400 | `rgb(24, 27, 33)` | `1px solid rgb(42, 46, 55)` | Click any lot on the map to add it (＋) — ×1 | src/workspaces/site-planner/MapFinder.jsx:4765:                ? "Click any lot on the map to add it (＋) — it works even before the purple outlines appear. Zoom in a little to see the lines." |
...
| 0px | 25px | 12px | 500 | `rgba(0, 0, 0, 0)` | `0px none rgb(69, 76, 92)` | Schedule, Review, Library (+2 more) ×5 | src/app/AccountControl.jsx:7: * once (Site map header + Site plan header + DocReview + Library + Scheduler), most of them (+more matches elsewhere, best-effort) |
| 8px | 30px | 12px | 400 | `rgb(234, 238, 243)` | `1px solid rgb(215, 221, 229)` | Full screen, Settings ×2 | src/shared/ui/AppHeader.jsx:153:      aria-label={active ? "Leave full screen" : "Full screen"} (+more matches elsewhere, best-effort) |
| 8px 0px 0px 8px | 26px | 12px | 500 | `rgba(0, 0, 0, 0)` | `1px solid rgba(0, 0, 0, 0)` | Undo, Redo ×2 | src/shared/comps/components/CompEntryGrid.jsx:767:  // summary line can say so and Undo can remove precisely those rows, nothing else. (+more matches elsewhere, best-effort) |
| 0px 8px 8px 0px | 26px | 12px | 500 | `rgba(0, 0, 0, 0)` | `1px solid rgba(0, 0, 0, 0)` | Recent actions to undo, Recent actions to redo ×2 | src/workspaces/site-planner/SitePlanner.jsx:20008:            aria-label="Recent actions to undo" title="Recent actions to undo"> |
| 6px | 23px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(0, 0, 0)` | Dashboard: all projects ×1 | src/shared/ui/dashboardNav.js:27:    title: action ? (logoDashboardTitle || "Dashboard: all projects") : undefined, |
...
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(27, 30, 38)` | Switch project ×1 | src/shared/ui/ProjectBreadcrumb.jsx:802:        title={cross ? "Browsing all projects" : org ? "Browsing your organization's notes, library and agenda" : currentProject ? "Switch project" : "Choose a project or organization"} (+more matches elsewhere, best-effort) |
| 8px | 30px | 12px | 500 | `rgba(0, 0, 0, 0)` | `0px none rgb(27, 30, 38)` | Switch or rename plan ×1 | src/workspaces/site-planner/SitePlanner.jsx:19768:          "Switch project sits 10.7px from Switch or rename plan — height 26px vs 24px" sibling (+more matches elsewhere, best-effort) |
...
| 8px | 30px | 12px | 600 | `rgb(255, 255, 255)` | `1px solid rgb(215, 221, 229)` | Sign in or create an account ×1 | src/app/AccountControl.jsx:226:      <MenuTrigger onClick={onOpenAuth} title="Sign in or create an account" caret={false} leading={<span style={avatar(false)}>›</span>} data-testid="account-signed-out"> |
| 0px | 25px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(15, 110, 86)` | Site ×1 | src/app/AccountControl.jsx:7: * once (Site map header + Site plan header + DocReview + Library + Scheduler), most of them (+more matches elsewhere, best-effort) |
| 8px | 26px | 12px | 600 | `rgba(0, 0, 0, 0)` | `1px solid rgb(215, 221, 229)` | File — export a PNG or print a PDF ×1 | src/workspaces/site-planner/SitePlanner.jsx:19959:            title="File — export a PNG or print a PDF" |
| 8px | 26px | 12px | 500 | `rgba(0, 0, 0, 0.06)` | `1px solid rgba(0, 0, 0, 0)` | Zoom to fit ×1 | src/workspaces/site-planner/MapFinder.jsx:266: * "+ Select parcels", "Turn all 1 layer off", "Zoom to fit", "Export to Google Earth (KMZ)" — none (+more matches elsewhere, best-effort) |
...
| 8px | 30px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(236, 239, 244)` | Switch project ×1 | src/shared/ui/ProjectBreadcrumb.jsx:802:        title={cross ? "Browsing all projects" : org ? "Browsing your organization's notes, library and agenda" : currentProject ? "Switch project" : "Choose a project or organization"} (+more matches elsewhere, best-effort) |
| 8px | 30px | 12px | 500 | `rgba(0, 0, 0, 0)` | `0px none rgb(236, 239, 244)` | Switch or rename plan ×1 | src/workspaces/site-planner/SitePlanner.jsx:19768:          "Switch project sits 10.7px from Switch or rename plan — height 26px vs 24px" sibling (+more matches elsewhere, best-effort) |
| 8px | 30px | 12px | 400 | `rgba(0, 0, 0, 0)` | `1px solid rgba(0, 0, 0, 0)` | Cloud sync: Saved on this device ×1 | unattributed (no source match — best-effort text search) |
| 8px | 30px | 12px | 600 | `rgb(23, 26, 33)` | `1px solid rgb(38, 42, 51)` | Sign in or create an account ×1 | src/app/AccountControl.jsx:226:      <MenuTrigger onClick={onOpenAuth} title="Sign in or create an account" caret={false} leading={<span style={avatar(false)}>›</span>} data-testid="account-signed-out"> |
| 0px | 25px | 12px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(93, 202, 165)` | Site ×1 | src/app/AccountControl.jsx:7: * once (Site map header + Site plan header + DocReview + Library + Scheduler), most of them (+more matches elsewhere, best-effort) |
| 8px | 26px | 12px | 600 | `rgba(0, 0, 0, 0)` | `1px solid rgb(38, 42, 51)` | File — export a PNG or print a PDF ×1 | src/workspaces/site-planner/SitePlanner.jsx:19959:            title="File — export a PNG or print a PDF" |
| 8px | 26px | 12px | 500 | `rgba(255, 255, 255, 0.09)` | `1px solid rgba(0, 0, 0, 0)` | Zoom to fit ×1 | src/workspaces/site-planner/MapFinder.jsx:266: * "+ Select parcels", "Turn all 1 layer off", "Zoom to fit", "Export to Google Earth (KMZ)" — none (+more matches elsewhere, best-effort) |
...
| radius | height | font | weight | background | border | label(s) | file/line (best-effort) |
|---|---|---|---|---|---|---|---|
| 6px | 24px | 12px | 500 | `rgba(0, 0, 0, 0)` | `0px none rgb(27, 30, 38)` | Save the current view as a PNG image, Pick a print frame, then download a finished PDF (no browser print dialog) ×2 | src/workspaces/site-planner/SitePlanner.jsx:19972:            <button style={menuItem(false)} title="Save the current view as a PNG image" onClick={() => { setExportMenu(false); exportPNG(); }}>Export PNG</button> |

**dark** — 1 distinct style signature(s) over 2 matched element(s):

| radius | height | font | weight | background | border | label(s) | file/line (best-effort) |
|---|---|---|---|---|---|---|---|
| 6px | 24px | 12px | 500 | `rgba(0, 0, 0, 0)` | `0px none rgb(232, 235, 240)` | Save the current view as a PNG image, Pick a print frame, then download a finished PDF (no browser print dialog) ×2 | src/workspaces/site-planner/SitePlanner.jsx:19972:            <button style={menuItem(false)} title="Save the current view as a PNG image" onClick={() => { setExportMenu(false); exportPNG(); }}>Export PNG</button> |
...
| 0px | 30px | 14px | 600 | `rgb(255, 255, 255)` | `0px none rgb(27, 30, 38)` | Zoom out, Zoom to fit ×2 | src/workspaces/doc-review/DocReview.jsx:2024:    { kind: "tool", id: "zoomOut", label: "Out", title: "Zoom out (or scroll the wheel over the sheet)", icon: <MkIcon id="zoomOut" />, onClick: () => zoom(1 / 1.2) }, (+more matches elsewhere, best-effort) |
| 0px | 52px | 10.5px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(53, 59, 73)` | Properties, Standards ×2 | src/app/HelpReportControl.jsx:136: * Properties sheet and Food's FoodMap.jsx already use — see `coarsePointer` below), never by (+more matches elsewhere, best-effort) |
...
| 0px | 32px | 12px | 700 | `rgba(0, 0, 0, 0)` | `0px none rgb(27, 30, 38)` | Layers — map data layers (flood, utilities, parcels, aerial…) ×1 | src/workspaces/site-planner/SitePlanner.jsx:23921:              <button onClick={() => setLayersOpen((o) => !o)} aria-expanded={layersOpen} aria-label="Layers — map data layers (flood, utilities, parcels, aerial…)" (+more matches elsewhere, best-effort) |
| 0px | 30px | 14px | 600 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | Zoom in ×1 | src/shared/sitePlans/components/SitePlansSection.jsx:391:          <Button size="sm" variant="ghost" onClick={() => onZoomToOverlay(o)}>Zoom in</Button> (+more matches elsewhere, best-effort) |
| 8px | 30px | 12px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(205, 211, 220)` | Help and report a problem ×1 | src/app/HelpReportControl.jsx:359:      aria-label="Help and report a problem" |
| 8px | 27px | 12px | 650 | `rgb(194, 65, 12)` | `1px solid rgba(0, 0, 0, 0)` | Select V ×1 | unattributed (no source match — best-effort text search) |
...
| 0px | 54px | 10.5px | 600 | `rgba(0, 0, 0, 0.06)` | `0px none rgb(27, 30, 38)` | Yield ×1 | src/shared/ui/Chip.jsx:3:/* Chip (FINAL UI SPEC — Yield panel + Pond inspector; promoted to shared/ui, NEW-COMPS-CARD, from (+more matches elsewhere, best-effort) |
...
| 999px | 19px | 10.5px | 700 | `rgba(0, 0, 0, 0)` | `1px solid rgb(194, 65, 12)` | Drainage → ×1 | src/workspaces/site-planner/SitePlanner.jsx:20454:                      <span style={{ color: PAL.muted, fontWeight: 600, fontSize: 10.5, whiteSpace: "nowrap" }}>in Drainage →</span> (+more matches elsewhere, best-effort) |
...
| 0px | 30px | 14px | 600 | `rgb(24, 27, 33)` | `0px none rgb(232, 235, 240)` | Zoom out, Zoom to fit ×2 | src/workspaces/doc-review/DocReview.jsx:2024:    { kind: "tool", id: "zoomOut", label: "Out", title: "Zoom out (or scroll the wheel over the sheet)", icon: <MkIcon id="zoomOut" />, onClick: () => zoom(1 / 1.2) }, (+more matches elsewhere, best-effort) |
| 0px | 52px | 10.5px | 600 | `rgba(0, 0, 0, 0)` | `0px none rgb(166, 173, 186)` | Properties, Standards ×2 | src/app/HelpReportControl.jsx:136: * Properties sheet and Food's FoodMap.jsx already use — see `coarsePointer` below), never by (+more matches elsewhere, best-effort) |
| 0px | 32px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(0, 0, 0)` | ▶Buildings1 · 75,600 SF, ▶Costsnot priced yet ×2 | unattributed (no source match — best-effort text search) |
...
| 0px | 32px | 12px | 700 | `rgba(0, 0, 0, 0)` | `0px none rgb(232, 235, 240)` | Layers — map data layers (flood, utilities, parcels, aerial…) ×1 | src/workspaces/site-planner/SitePlanner.jsx:23921:              <button onClick={() => setLayersOpen((o) => !o)} aria-expanded={layersOpen} aria-label="Layers — map data layers (flood, utilities, parcels, aerial…)" (+more matches elsewhere, best-effort) |
| 0px | 30px | 14px | 600 | `rgb(24, 27, 33)` | `1px solid rgb(42, 46, 55)` | Zoom in ×1 | src/shared/sitePlans/components/SitePlansSection.jsx:391:          <Button size="sm" variant="ghost" onClick={() => onZoomToOverlay(o)}>Zoom in</Button> (+more matches elsewhere, best-effort) |
| 8px | 30px | 12px | 400 | `rgb(29, 32, 39)` | `1px solid rgb(58, 63, 75)` | Help and report a problem ×1 | src/app/HelpReportControl.jsx:359:      aria-label="Help and report a problem" |
| 8px | 27px | 12px | 650 | `rgb(242, 107, 58)` | `1px solid rgba(0, 0, 0, 0)` | Select V ×1 | unattributed (no source match — best-effort text search) |
...
| 0px | 54px | 10.5px | 600 | `rgba(255, 255, 255, 0.09)` | `0px none rgb(236, 239, 244)` | Yield ×1 | src/shared/ui/Chip.jsx:3:/* Chip (FINAL UI SPEC — Yield panel + Pond inspector; promoted to shared/ui, NEW-COMPS-CARD, from (+more matches elsewhere, best-effort) |
...
| 0px | 31px | 12px | 400 | `rgba(0, 0, 0, 0)` | `0px none rgb(0, 0, 0)` | ▶Land use ×1 | unattributed (no source match — best-effort text search) |
| 999px | 19px | 10.5px | 700 | `rgba(0, 0, 0, 0)` | `1px solid rgb(242, 107, 58)` | Drainage → ×1 | src/workspaces/site-planner/SitePlanner.jsx:20454:                      <span style={{ color: PAL.muted, fontWeight: 600, fontSize: 10.5, whiteSpace: "nowrap" }}>in Drainage →</span> (+more matches elsewhere, best-effort) |
...
| radius | height | font | weight | background | border | label(s) | file/line (best-effort) |
|---|---|---|---|---|---|---|---|
| 8px | 30px | 12px | 600 | `rgb(255, 255, 255)` | `1px solid rgb(225, 229, 235)` | Open PDF…, ⇄ Compare revisions… ×2 | src/workspaces/doc-review/DocReview.jsx:681:    if (!file) { setOpenErr("No file was received from that drop. Try the Open PDF… button, or drop a single .pdf."); return; } (+more matches elsewhere, best-effort) |
| 8px | 30px | 12px | 700 | `rgb(14, 116, 144)` | `1px solid rgb(14, 116, 144)` | 🗂 Browse the Library ×1 | src/workspaces/doc-review/DocReview.jsx:2313:                🗂 Browse the Library |
| 8px | 30px | 12px | 400 | `rgb(255, 255, 255)` | `1px solid rgb(205, 211, 220)` | Help and report a problem ×1 | src/app/HelpReportControl.jsx:359:      aria-label="Help and report a problem" |
...
| radius | height | font | weight | background | border | label(s) | file/line (best-effort) |
|---|---|---|---|---|---|---|---|
| 8px | 30px | 12px | 600 | `rgb(29, 32, 39)` | `1px solid rgb(42, 46, 55)` | Open PDF…, ⇄ Compare revisions… ×2 | src/workspaces/doc-review/DocReview.jsx:681:    if (!file) { setOpenErr("No file was received from that drop. Try the Open PDF… button, or drop a single .pdf."); return; } (+more matches elsewhere, best-effort) |
| 8px | 30px | 12px | 700 | `rgb(14, 116, 144)` | `1px solid rgb(14, 116, 144)` | 🗂 Browse the Library ×1 | src/workspaces/doc-review/DocReview.jsx:2313:                🗂 Browse the Library |
| 8px | 30px | 12px | 400 | `rgb(29, 32, 39)` | `1px solid rgb(58, 63, 75)` | Help and report a problem ×1 | src/app/HelpReportControl.jsx:359:      aria-label="Help and report a problem" |
