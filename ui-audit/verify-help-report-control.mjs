 ...
 *
 * ⛔ B1162016 (owner report, 2026-09-07) — PART H proves the control sizes by POINTER
...
try {
  ...
  // ─────────────────────────────────────────── PART H — B1162016: size by POINTER CAPABILITY,
  // never viewport width, and the corner-clearance math consumes the SAME size the button renders at.
  console.log("\nPART H — the control sizes by POINTER CAPABILITY (matchMedia \"(pointer: coarse)\"), never viewport width, and the corner-clearance math consumes the size the button actually rendered at");
  {
    ...
    async function openChromeFreeScreen(ctxOpts) {
      ...
      const page = await ctx.newPage();
      await assertMeasurable(page, "verify-help-report-control PART H");
    ...
    // vw-45=1395, 12px wide (left edge 1383) — 1395 is > 1382 but not > 1396.
    {
      ...
      const page = await ctx.newPage();
      await assertMeasurable(page, "verify-help-report-control PART H (live switch)");
      await page.goto(URL + "#/schedule", { waitUntil: "load" });
      await page.waitForSelector('[data-testid="help-report-fab"]', { timeout: 15000 });
      await pacedWait(page, 400);
