...
// ------------------------------------------------------------------------------------------
// AUTH-STATE GATE (NEW-15 / B846608). The whole point of this crawl is to measure what CI's
// required `build` check actually renders — and CI's Build step (.github/ci-gates.yml) always
// carries REAL `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` secrets (owner-provisioned,
// 2026-06-22), which Vite inlines as build-time string literals (site-planner/lib/supabase.js's
// `supabaseConfigured()`). So CI's canonical, logged-out state is "Supabase configured, nobody
// signed in" — AccountControl.jsx's "Sign in or create an account" pill. A build made WITHOUT
// those two env vars set (the default on a bare `npx vite build` with no secrets exported) falls
// through to a DIFFERENT branch of the same component — the "Cloud sync isn't set up" pill —
// which is a real, correct rendering of a real, different app state, just never the one CI
// measures.
//
// PR #1397/#1398 (both 2026-09-03) failed the Signature-budget gate on exactly this: the
// committed docs/UI-INVENTORY.md held the unconfigured pill's copy (generated by a session that
// built without the two env vars), CI's build genuinely has them, so the required check's own
// regen disagreed on ~25 lines that had nothing to do with either PR's diff. `ci-parity.mjs`
// already closes this gap for a session that regenerates THROUGH it (it substitutes a truthy
// dummy value when the real secret is absent — see that script's own header, B927104) — but
// nothing stopped a session from running this script directly against a preview server it built
// by hand, without the substitution, and committing the wrong state with no warning at all. This
// closes that gap AT THE GENERATOR, unconditionally (both `--check` and regen mode), so it
// cannot recur regardless of how the script is invoked: refuse to write anything until the crawl
// itself confirms it is looking at CI's canonical branch.
//
// `classifyAuthState` is the pure decision (unit-tested in test/uiInventoryAuthGate.test.js);
// `assertCanonicalAuthState` is the one-time browser probe that feeds it, reading the same
// copy-independent `data-testid` hooks AccountControl.jsx now carries for exactly this purpose —
// a title-TEXT match would silently break the next time that copy is edited, which is the exact
// class of bug this gate exists to catch.
// ------------------------------------------------------------------------------------------
export function classifyAuthState({ cloudOff, signedOut }) {
  if (signedOut && !cloudOff) {
    return { ok: true, state: "signed-out-configured", message: null };
  }
  if (cloudOff && !signedOut) {
    return {
      ok: false,
      state: "cloud-off",
      message:
        "ui-inventory.mjs refused to run: this build was made with Supabase NOT configured " +
        "(VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY empty or absent at `vite build` time), so " +
        "AccountControl renders its \"Cloud sync isn't set up\" pill. CI's required `build` check " +
        "always builds with real secrets configured (see .github/ci-gates.yml's Build step), so " +
        "its canonical logged-out state is the \"Sign in or create an account\" pill instead — a " +
        "different branch of the same component. Regenerating docs/UI-INVENTORY.md from an " +
        "unconfigured build produces a file CI's own check will always reject, on unrelated PRs, " +
        "for as long as it stays committed (PR #1397/#1398, 2026-09-03).\n\n" +
        "Fix: rebuild with BOTH env vars set to any truthy value before running this script — a " +
        "dummy value matches CI's SHAPE, never its real secret:\n" +
        '  VITE_SUPABASE_URL="https://ui-inventory.supabase.co" \\\n' +
        '    VITE_SUPABASE_ANON_KEY="ui-inventory-dummy-key" \\\n' +
        "    npx vite build && npx vite preview --port 4173\n" +
        "…or just run `npm run ci-parity`, which substitutes this automatically and prints when " +
        "it does (NEW-15 / B846608).",
    };
  }
  return {
    ok: false,
    state: "unknown",
    message:
      "ui-inventory.mjs refused to run: could not determine the crawl's signed-out auth state on " +
      '#/site — expected exactly one of [data-testid="account-signed-out"] (Supabase configured) ' +
      'or [data-testid="account-cloud-off"] (Supabase unconfigured) to be present, found ' +
      `{ signedOut: ${signedOut}, cloudOff: ${cloudOff} }. Either AccountControl.jsx's markup ` +
      "changed (update those data-testid hooks or this assertion to match), or the crawl is " +
      "somehow signed in (it never seeds a session, so that would itself be a bug) " +
      "(NEW-15 / B846608).",
  };
}

async function assertCanonicalAuthState(browser) {
  const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 }, deviceScaleFactor: 1 });
  await ctx.addInitScript(seed("light"));
  const page = await ctx.newPage();
  try {
    await assertMeasurable(page, "ui-inventory-auth-gate");
    await page.goto(BASE + "#/site", { waitUntil: "load" });
    await page.waitForTimeout(1200);
    const flags = await page.evaluate(() => ({
      cloudOff: !!document.querySelector('[data-testid="account-cloud-off"]'),
      signedOut: !!document.querySelector('[data-testid="account-signed-out"]'),
    }));
    const verdict = classifyAuthState(flags);
...
// NEW-15 / B846608 — guarded rather than a bare top-level call, so `classifyAuthState` (and
// `checkSignatureBudget`, already exported) can be imported by a unit test with no side effect —
// importing this module used to unconditionally launch a real Chromium crawl.
