...
const COUNTIES_MAP_RAW = {
   ...
   *
   * County boundary POLYGONS remain TX + CO only (`countyPolygons.js`, 318 total) — none of these
   * 25 states gets a boundary outline from that asset. `candidateCountiesForPoint`'s geometry-first
   * hoist (`geometryCountyKey`) and `countyForView`'s jurisdiction badge are untouched by this
   * change and keep their existing TX/CO-only behaviour for every one of these states; a click here
   * still resolves a PARCEL via the layer below, it just doesn't get the geometry-verified county
   * hoist Texas/Colorado sites do.
   *
   * ⛔ DECIDED, NOT A GAP (NEW-2, 2026-09-08) — read this before re-flagging it a fifth time. Every
   * wiring pass through this file (the original 19 · the HI/MD/NE/NH continuation · VA/WV here) has
   * named this same absence on its own line and none has acted on it, because each pass's scope was
   * "find this state a parcel source", never "source every state's own county-boundary GIS layer"
   * too — a real, separate research-and-build project (each state publishes its own county-boundary
   * service, under its own agency, own field names, own quirks — exactly the per-state groundwork
   * `probe-statewide-parcels.mjs` just did for parcels, times 25, for a DIFFERENT layer). Extending
   * `scripts/build-county-polygons.mjs` state-by-state is real, worthwhile follow-up work; it is not
   * one session's work, and pretending otherwise here would either ship nothing or ship an
   * unresearched guess at a boundary source. So the decision, stated plainly: **county outline
   * polygons stay a Texas/Colorado-only feature until a future item scopes the state-by-state build
   * for the rest.** The next session that wants it opens a scoped item and does that research: it
   * does not silently re-note this paragraph a sixth time.
   *
