...
export async function fetchSiteSummaries() {
  ...
  try {
    const { data, error } = await supabase
      .from("sites")
      .select("id, group_id, site, name, county, created_at, updated_at, thumbnail_svg, status:data->>status, role:data->>role, siteRenamedAt:data->>siteRenamedAt, origin:data->origin")
      .is("deleted_at", null)
