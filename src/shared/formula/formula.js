...
const tokenize = src => {
  ...
  while (i < n) {
    ...
    // Sheet-name literal: 'Sheet Name'  — single-quoted, '' as an escaped quote (mirrors the
    // double-quote string literal above). Cross-sheet references (below): a bare identifier
    // needs no quoting (Sheet1!A1); a name with spaces/punctuation must be quoted
    // ('My Sheet'!A1), matching Excel's own rule. This token is meaningful ONLY directly
    // before "!" — the parser rejects it anywhere else — but is tokenized unconditionally
    // here since quotes have no other role in this grammar.
    if (c === "'") {
      let j = i + 1, out = "";
      while (j < n) {
        if (s[j] === "'") { if (s[j + 1] === "'") { out += "'"; j += 2; continue; } break; }
      ...
      if (j >= n) throw ferr(FORMULA_ERRORS.ERR, "unterminated sheet name");
      toks.push({ t: "sheetq", v: out, pos: i }); i = j + 1; continue;
    }
    ...
    // "!" is the cross-sheet qualifier separator (Sheet1!A1) — no other meaning in this grammar.
...
const parse = toks => {
  ...
  // the same way regardless of which side(s) carry a "$". `sheet` (added for
  // cross-sheet references, below) is the qualifying sheet NAME as typed — Sheet1!A1:B10
  // qualifies the WHOLE range, never just its first endpoint — or null for a plain,
  // same-sheet reference (unchanged default, so every pre-existing call site is untouched).
  const finishRefOrRange = (first, sheet = null) => {
    if (peek().t === "op" && peek().v === ":") {
      next();
      const t2 = peek();
      ...
      next();
      return { type: "range", sheet, from: first, to: second };
    }
    return { type: "ref", sheet, ...first };
  };
  const parseAtom = () => {
    const t = peek();
    // Cross-sheet qualifier: Sheet1!A1 or 'Sheet With Spaces'!A1 — an id or a quoted
    // sheet-name token directly followed by "!" and then a cell reference. Checked FIRST,
    // ahead of the ordinary "id" dispatch below, since "!" has no other role in this
    // grammar and can never collide with a function call (which needs "(", not "!").
    if ((t.t === "id" || t.t === "sheetq") && toks[p + 1] && toks[p + 1].t === "op" && toks[p + 1].v === "!") {
      const sheetName = t.v;
      next(); next(); // consume the name token + "!"
      const t2 = peek();
      if (t2.t !== "ref" && t2.t !== "id") throw ferr(FORMULA_ERRORS.ERR, `expected a cell reference after sheet name "${sheetName}!"`);
      const info = parseRefText(t2.v);
      if (!info) throw ferr(FORMULA_ERRORS.NAME, `unknown name "${t2.v}"`);
      next();
      return finishRefOrRange(info, sheetName);
    }
    ...
    if (t.t === "sheetq") throw ferr(FORMULA_ERRORS.ERR, `a quoted sheet name must be followed by "!"`);
    if (t.t === "ref") {
    ...
    if (t.t === "id") {
      ...
      // (out-of-bounds addresses like ABCD1/XFE1/A0/A1048577 land here too) is a NAMED
      // RANGE reference (Model workspace, NEW-1) — deferred to EVAL time via ctx.names,
      // exactly the way a [Column] reference already defers to ctx.columns, rather than
      // resolved here at parse time. A host that never wires up ctx.names (the Schedule
      // module, Cost Estimating, or a Model formula naming a name that doesn't exist)
      // throws the SAME #NAME? this used to throw right here — see the "name" case in
      // evalNode below — so nothing about the existing parse-time contract changes for
...
function rangeArray(node, ctx) {
  // node.sheet resolves ONCE here (throwing on an unknown sheet, same as a scalar "ref"
  // would) rather than once per cell in the loop below — an unresolvable sheet name is a
  // property of the WHOLE range, not of any one cell in it.
  const grid = node.sheet ? resolveSheetGrid(ctx, node.sheet) : ctx.grid;
...
// order). An A1-shaped range argument (A1:B10) is handled by rangeArray above. A
// NAMED RANGE argument (Model workspace, NEW-1 — e.g. SUM(LandCosts)) resolves to its
// own rectangle and reuses rangeArray over that — a single-cell name contributes a
// one-element array, exactly like a one-cell A1:A1 range would.
...
// Shared by the four collectors below, so SUM/MIN/MAX/AVERAGE/COUNT/COUNTA treat
// "[Column]", "A1:B10" and a named range identically wherever one of them is legal —
...
// Cross-sheet references (Sheet1!A1) resolve through ctx.grids — an optional map of
// lowercased sheet name -> that sheet's own grid[][], wired by a multi-sheet host (the Model
// workbook evaluator) the same way ctx.grid itself is wired for the CURRENT sheet. A host that
// never wires ctx.grids (single-sheet, e.g. the scheduler's structured-only [Column] use)
// simply can't have typed a sheet-qualified reference in the first place; if one somehow
// appears, "unknown sheet" is the correct read, not a silent blank — a #REF!/never-a-crash
// contract like the rest of this engine, but a genuinely unresolvable NAME is a real error,
// not an empty cell (that's what an out-of-range cell WITHIN a resolved sheet still reads as).
function resolveSheetGrid(ctx, sheetName) {
  const key = String(sheetName).trim().toLowerCase();
  if (!ctx.grids || !Object.prototype.hasOwnProperty.call(ctx.grids, key)) throw ferr(FORMULA_ERRORS.REF, `unknown sheet "${sheetName}"`);
  return ctx.grids[key];
}
function readGridCell(ctx, row, col, sheetName) {
  const grid = sheetName ? resolveSheetGrid(ctx, sheetName) : ctx.grid;
  if (!grid) return undefined;
  const r = grid[row - 1];
  if (!r) return undefined;
  return r[col - 1];
}
// A NAMED RANGE reference (Model workspace, NEW-1). ctx.names is an optional map: lowercased
// name -> {r1,c1,r2,c2} (1-based, inclusive — a single-cell name has r1===r2 && c1===c2, the
// same shape the Model workspace's lib/namedRanges.js stores). A host that never wires one up
...
const evalNode = (node, ctx) => {
  switch (node.type) {
    ...
    case "ref": {
      const v = readGridCell(ctx, node.row, node.col, node.sheet);
    ...
    // A NAMED RANGE (Model workspace, NEW-1) reached as a SCALAR — e.g. "=LandCost*2". A
    // name defined over a single cell reads that cell, exactly like a bare "ref"; a name
    // defined over several cells is the same VALUE error a multi-cell "range" node
    ...
    case "name": {
      ...
      throw ferr(FORMULA_ERRORS.VALUE, `"${node.name}" is a multi-cell named range and cannot be used where a single value is expected`);
    }
...
const evaluateFormula = (src, ctx) => {
  ...
  const fullCtx = {
    ...
    // grids powers CROSS-SHEET A1 references (Sheet1!A1) — an optional map of lowercased
    // sheet name -> that sheet's own grid[][], wired only by a multi-sheet host (the Model
    // workbook evaluator). Absent (null) for every other caller — a sheet-qualified
    // reference then reads as "unknown sheet" (#REF!), never a silent blank.
    grids: (ctx && ctx.grids) || null,
    // names powers NAMED RANGE references (Model workspace, NEW-1). Optional map, lowercased
    // name -> {r1,c1,r2,c2} (1-based). A host that never wires one up (Schedule, Cost
    // Estimating) leaves every bare "name" AST node unresolvable — resolveNamedRange throws
    // #NAME? for it, exactly the error the parser itself used to throw at parse time before
    // named ranges existed.
    names: (ctx && ctx.names) || null,
...
//
// ── Cross-sheet qualifiers (Model workbook Stage 3, NEW-1) ──────────────────────
// A reference can carry a leading "Sheet1!" or "'Sheet One'!" qualifier (tokenizes as an
// "id"/"sheetq" token immediately followed by an "op":"!" token — see the tokenizer above).
// `sheetQualifierAt` is the ONE shared detector every token-level rewrite below uses so they
// can never disagree about what counts as a qualifier; each rewrite then decides what to DO
// with one:
//   - rewriteFormulaForCopy / rewriteFormulaForStructuralShift: the qualifier's own text is
//     copied through byte-for-byte (a copy/fill/insert/delete never changes WHICH sheet a
//     reference points at) — only the ref/range span after "!" is rewritten, exactly like an
//     unqualified reference.
//   - rewriteFormulaForSheetRename: the mirror image — ONLY the qualifier's own text changes
//     (to the sheet's new name), the ref/range after it is untouched.
//   - dropFormulaSheetRefs: the whole qualified span (qualifier and ref/range both) becomes
//     the literal "#REF!" — there is no sheet left to keep a qualifier for.
function sheetQualifierAt(toks, idx) {
  const t = toks[idx];
  if (!t || (t.t !== "id" && t.t !== "sheetq")) return null;
  const bang = toks[idx + 1];
  if (!bang || bang.t !== "op" || bang.v !== "!") return null;
  return { name: t.v, refIdx: idx + 2 };
}
const rewriteFormulaForCopy = (formulaSrc, sourceAddr, targetAddr) => {
  ...
  for (let i = 0; i < toks.length; i++) {
    const t = toks[i];
    if (t.t === "eof") continue;
    const qual = sheetQualifierAt(toks, i);
    const refIdx = qual ? qual.refIdx : i;
    const refTok = toks[refIdx];
    if (!refTok || !isRefEligible(refTok, refIdx)) continue;
    const isRange = toks[refIdx + 1] && toks[refIdx + 1].t === "op" && toks[refIdx + 1].v === ":" &&
      toks[refIdx + 2] && isRefEligible(toks[refIdx + 2], refIdx + 2);
    const endTok = isRange ? toks[refIdx + 2] : refTok;
    const spanStart = t.pos, spanEnd = endTok.pos + String(endTok.v).length;
    let refReplacement;
    if (isRange) {
      const from = shift(refTok.v), to = shift(endTok.v);
      refReplacement = (from && to) ? `${from}:${to}` : "#REF!";
    } else {
      refReplacement = shift(refTok.v) || "#REF!";
    }
    const qualifierText = qual ? text.slice(t.pos, refTok.pos) : "";
    out += text.slice(cursor, spanStart) + qualifierText + refReplacement;
    cursor = spanEnd;
    i = isRange ? refIdx + 2 : refIdx;
  }
...
//
// ⛔ CROSS-SHEET SCOPING (Model workbook Stage 3, NEW-1) — `ownerSheetName` (the sheet the
// FORMULA ITSELF lives on) and `editedSheetName` (the sheet whose rows/columns are actually
// being inserted/deleted) decide, PER REFERENCE, whether this call's shift even applies to
// it. A reference with no qualifier means "this row/column ON THE SHEET THE FORMULA LIVES
// ON" (Excel semantics — a bare A1 typed in Sheet2 means Sheet2!A1), so its EFFECTIVE sheet
// is `ownerSheetName`; a qualified reference's effective sheet is its own qualifier. Only a
// reference whose effective sheet matches `editedSheetName` shifts — everything else (a bare
// ref in a formula living on some OTHER sheet, or a qualified ref naming a THIRD sheet) is
// copied through untouched, because the sheet that actually restructured is not the sheet
// that reference addresses. Both params default to `null`, and passing neither preserves the
// EXACT original single-sheet behavior (every reference shifts, unconditionally) — every
// caller from before cross-sheet support (including the Schedule module's mirrored copy,
// which never sets these) gets byte-identical results.
const rewriteFormulaForStructuralShift = (formulaSrc, axis, at, delta, ownerSheetName = null, editedSheetName = null) => {
  const text = String(formulaSrc == null ? "" : formulaSrc);
  let toks;
  try { toks = tokenize(text); }
  catch { return text; } // doesn't even lex — nothing this transform can safely touch
  ...
  const sameSheet = name => editedSheetName == null || String(name || "").trim().toLowerCase() === String(editedSheetName).trim().toLowerCase();
  let out = "", cursor = 0;
  for (let i = 0; i < toks.length; i++) {
    const t = toks[i];
    if (t.t === "eof") continue;
    ...
    const refTok = toks[refIdx];
    if (!refTok || !isRefEligible(refTok, refIdx)) continue;
    const effectiveSheet = qual ? qual.name : ownerSheetName;
    const isRange = toks[refIdx + 1] && toks[refIdx + 1].t === "op" && toks[refIdx + 1].v === ":" &&
      toks[refIdx + 2] && isRefEligible(toks[refIdx + 2], refIdx + 2);
    const endTok = isRange ? toks[refIdx + 2] : refTok;
    const spanStart = t.pos, spanEnd = endTok.pos + String(endTok.v).length;
    const qualifierText = qual ? text.slice(t.pos, refTok.pos) : "";
    let refReplacement;
    if (!sameSheet(effectiveSheet)) {
      // This structural edit is on a DIFFERENT sheet than the one this reference
      // addresses — copy the ref/range through byte-for-byte, unshifted.
      refReplacement = text.slice(refTok.pos, spanEnd);
    } else {
      const info1 = parseRefText(refTok.v);
      ...
      if (!iv) {
        ...
        refReplacement = shiftRef(refTok.v, iv.min);
      } else {
        ...
        refReplacement = `${shiftRef(refTok.v, fromVal)}:${shiftRef(endTok.v, toVal)}`;
      }
    }
    out += text.slice(cursor, spanStart) + qualifierText + refReplacement;
    cursor = spanEnd;
...
// rewriteFormulaForSheetRename: rewrite every reference QUALIFIED to `oldSheetName` so it
// names `newSheetName` instead — a renamed sheet must not silently break every formula that
// points at it (Model workbook Stage 3, NEW-1). Token-level, same technique as the two
// transforms above — ONLY the qualifier's own span changes; the ref/range that follows it is
// completely untouched, and an UNQUALIFIED reference is left alone outright (it belongs to
// whichever sheet the formula itself lives in — renaming some OTHER sheet can never affect
// it). `newSheetName` is quoted, with an embedded "'" doubled, exactly when it doesn't lex as
// a single bare identifier — the same rule the tokenizer/parser use to decide whether typing
// it back in would even need quotes.
const BARE_SHEET_NAME = /^[A-Za-z_][A-Za-z0-9_.]*$/;
function formatSheetQualifier(name) {
  return (BARE_SHEET_NAME.test(name) ? name : `'${String(name).replace(/'/g, "''")}'`) + "!";
}
const rewriteFormulaForSheetRename = (formulaSrc, oldSheetName, newSheetName) => {
  const text = String(formulaSrc == null ? "" : formulaSrc);
  let toks;
  try { toks = tokenize(text); }
  catch { return text; }
  ...
  const oldLower = String(oldSheetName).trim().toLowerCase();
  const replacement = formatSheetQualifier(newSheetName);
  let out = "", cursor = 0;
  for (let i = 0; i < toks.length; i++) {
    const qual = sheetQualifierAt(toks, i);
    if (!qual || qual.name.trim().toLowerCase() !== oldLower) continue;
    const refTok = toks[qual.refIdx];
    if (!refTok || !isRefEligible(refTok, qual.refIdx)) continue; // not really a reference — leave the text alone
    const nameTok = toks[i], bangTok = toks[i + 1];
    out += text.slice(cursor, nameTok.pos) + replacement;
    cursor = bangTok.pos + 1; // just past the "!"
  }
  out += text.slice(cursor);
  return out;
};

// dropFormulaSheetRefs: turn every reference qualified to `deletedSheetName` into the literal
// text "#REF!" — the WHOLE qualified span, qualifier included, since a reference to a sheet
// that no longer exists has no meaningful qualifier left to keep (Model workbook Stage 3,
// NEW-1 — Excel's own behavior when a referenced sheet is deleted). An unqualified reference
// is untouched (it belongs to whichever sheet the formula itself lives in, never the one
// being deleted — deleting Sheet2 does not corrupt a bare A1 in a Sheet2 formula's own
// sibling cells... but the deleted sheet's own cells are simply removed by the caller, so
// this transform is applied only to every OTHER sheet's formulas).
const dropFormulaSheetRefs = (formulaSrc, deletedSheetName) => {
  const text = String(formulaSrc == null ? "" : formulaSrc);
  let toks;
  try { toks = tokenize(text); }
  catch { return text; }
  ...
  const deletedLower = String(deletedSheetName).trim().toLowerCase();
  let out = "", cursor = 0;
  for (let i = 0; i < toks.length; i++) {
    const qual = sheetQualifierAt(toks, i);
    if (!qual || qual.name.trim().toLowerCase() !== deletedLower) continue;
    const refIdx = qual.refIdx;
    const refTok = toks[refIdx];
    if (!refTok || !isRefEligible(refTok, refIdx)) continue;
    const isRange = toks[refIdx + 1] && toks[refIdx + 1].t === "op" && toks[refIdx + 1].v === ":" &&
      toks[refIdx + 2] && isRefEligible(toks[refIdx + 2], refIdx + 2);
    const endTok = isRange ? toks[refIdx + 2] : refTok;
    const spanStart = toks[i].pos, spanEnd = endTok.pos + String(endTok.v).length;
    out += text.slice(cursor, spanStart) + "#REF!";
    cursor = spanEnd;
    i = isRange ? refIdx + 2 : refIdx;
  }
  out += text.slice(cursor);
